...
#define LOG_AST(format, args...)    pr_alert(MyTag "[%s, line%05d]ASSERT:" format "\n", __func__, __LINE__, ##args)
...
/*
  *   if CONFIG_MTK_SMI_VARIANT is defined, isp scpsys is controlled by smi for suspend/resume.
  * otherwise, isp driver control scpsys by power-domain apis
*/
/*#undef CONFIG_MTK_SMI_VARIANT*/
...
#define ISP_MIPI_ANA_ADDR               0x10011800
#endif
...
/* ISP_IRQ_INTX_STATUS_BPC_ERR_ST  */
...
static MUINT32 g_oldImgoAddr = DEFAULT_PA;
static MUINT32 g_newImgoAddr = DEFAULT_PA;
static MUINT32 g_oldImg2oAddr = DEFAULT_PA;
static MUINT32 g_newImg2oAddr = DEFAULT_PA;
...
#define RegDump(start, end) {\
	...
	for (i = start; i <= end; i += 0x4) {\
		LOG_ERR("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + i), (unsigned int)ISP_RD32(ISP_ADDR + i));\
	} \
}

static void ISP_DumpBasicRegisters(void)
{
	unsigned int value, value1, value2;
	unsigned int sensor_width, sensor_height, tg_pxl_start, tg_pxl_end, tg_line_start,
	    tg_line_end;
	unsigned int tg_from_seninf_width, tg_from_seninf_height;
	unsigned int sensor_type, is_yuv = 0;

	LOG_INF("%s Start.", __func__);
	/* Seninf status */
	value = ISP_RD32(ISP_ADDR + 0x43B4);
	if (value != 0x1004) {
		LOG_ERR
		    ("If MIPI sensor, 83B4 Should be 0x1004, but:%08X:  Please check Sensor MIPI signal!",
		     value);
	}

	/* Sensor Type: Raw or YUV */
	value = ISP_RD32(ISP_ADDR + 0x010);
	sensor_type = value1 = (value & 0x000F0000) >> 16;	/* Bit16~19 */

	is_yuv = (value1 == 3) ? 1 : 0;
	LOG_INF("4010: %08X, is %s Sensor.", value,
		sensor_type == 1 ? "Raw" : (sensor_type == 3 ? "YUV" : "Unknown!"));
	if (sensor_type != 1 && sensor_type != 3)
		LOG_ERR("Sensor Type:%d(should be: 1: Raw, 3: YUV), Fatal Error!", sensor_type);

	/* Sensor Output Size */
	value = ISP_RD32(ISP_ADDR + 0x4134);

	sensor_width = value >> 16;
	if (is_yuv)
		sensor_width >>= 1;	/* YUV width should be divided by 2 */
	sensor_width &= 0xFFFF;

	sensor_height = value & 0xFFFF;

	LOG_INF("Sensor Output: width:%04X(%u), height:%04X(%u)", sensor_width, sensor_width,
		sensor_height, sensor_height);

	/*TG receive from seninf */
	value1 = ISP_RD32(ISP_ADDR + 0x448);
	tg_from_seninf_width = value1 >> 16;
	tg_from_seninf_height = value1 & 0x00001FFF;

	if (is_yuv)
		tg_from_seninf_width >>= 1;
	if (tg_from_seninf_height != 0)
		--tg_from_seninf_height;	/* YUV height should minus 1 */

	LOG_INF("TG receive from Seninf: %08X, width:%04X(%u), height:%04X(%u)",
		value1, tg_from_seninf_width, tg_from_seninf_width, tg_from_seninf_height,
		tg_from_seninf_height);

	if (tg_from_seninf_width != sensor_width)
		LOG_ERR("TG receive width(%u) from seninf != Sensor output width(%u)!",
			tg_from_seninf_width, sensor_width);
	if (tg_from_seninf_height != sensor_height)
		LOG_ERR("TG receive height(%u) from seninf != Sensor output height(%u)!",
			tg_from_seninf_height, sensor_height);

	/* TG Config */
	value1 = ISP_RD32(ISP_ADDR + 0x418);
	value2 = ISP_RD32(ISP_ADDR + 0x41C);

	tg_pxl_start = value1 & 0x00007FFF;
	tg_pxl_end = value1 >> 16;
	tg_line_start = value2 & 0x00001FFF;
	tg_line_end = value2 >> 16;
	LOG_INF("TG: pxl start:%04X(%u), pxl end:%04X(%u), line start:%04X(%u), line end:%04X(%u)",
		tg_pxl_start, tg_pxl_start, tg_pxl_end, tg_pxl_end, tg_line_start, tg_line_start,
		tg_line_end, tg_line_end);
	if (sensor_width < tg_pxl_end || tg_pxl_end - tg_pxl_start > sensor_width) {
		LOG_ERR
		    ("Sensor Output W(%u) should > TG max W(%u) or TG grab window W(%d) should <= Sensor W(%u)",
		     sensor_width, tg_pxl_end, tg_pxl_end - tg_pxl_start, sensor_width);
	}
	if (sensor_height < tg_line_end || tg_line_end - tg_line_start > sensor_height) {
		LOG_ERR
		    ("Sensor Output H(%u) should > TG max H(%u) or TG grab window H(%d) should <= Sensor H(%u)",
		     sensor_height, tg_line_end, tg_line_end - tg_line_start, sensor_height);
	}
...
static MINT32 ISP_DumpReg(MVOID)
{
	...
	/*  */
	ISP_DumpBasicRegisters();

	/* for tpipe main start */
	LOG_ERR("start MT");
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x000, ISP_RD32((void *)(ISP_ADDR + 0x000)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x004, ISP_RD32((void *)(ISP_ADDR + 0x004)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x008, ISP_RD32((void *)(ISP_ADDR + 0x008)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x00C, ISP_RD32((void *)(ISP_ADDR + 0x00C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x010, ISP_RD32((void *)(ISP_ADDR + 0x010)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x018, ISP_RD32((void *)(ISP_ADDR + 0x018)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x01C, ISP_RD32((void *)(ISP_ADDR + 0x01C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x020, ISP_RD32((void *)(ISP_ADDR + 0x020)));

#if 1				/* it may touch ReadClear register */
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x024, ISP_RD32((void *)(ISP_ADDR + 0x024)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x028, ISP_RD32((void *)(ISP_ADDR + 0x028)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x02C, ISP_RD32((void *)(ISP_ADDR + 0x02C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x030, ISP_RD32((void *)(ISP_ADDR + 0x030)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x034, ISP_RD32((void *)(ISP_ADDR + 0x034)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x038, ISP_RD32((void *)(ISP_ADDR + 0x038)));
#else
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x024, 0);
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x028, 0);
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x02C, 0);
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x030, 0);
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x034, 0);
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x038, 0);
#endif

	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x050, ISP_RD32((void *)(ISP_ADDR + 0x050)));	/* Tile setting */
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x074, ISP_RD32((void *)(ISP_ADDR + 0x074)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x078, ISP_RD32((void *)(ISP_ADDR + 0x078)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x07C, ISP_RD32((void *)(ISP_ADDR + 0x07C)));
#if 0				/* WO */
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x080, ISP_RD32((void *)(ISP_ADDR + 0x080)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x084, ISP_RD32((void *)(ISP_ADDR + 0x084)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x088, ISP_RD32((void *)(ISP_ADDR + 0x088)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x08C, ISP_RD32((void *)(ISP_ADDR + 0x08C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x090, ISP_RD32((void *)(ISP_ADDR + 0x090)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x094, ISP_RD32((void *)(ISP_ADDR + 0x094)));
#endif
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x0A8, ISP_RD32((void *)(ISP_ADDR + 0x0A8)));	/* cq0 base */
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x0AC, ISP_RD32((void *)(ISP_ADDR + 0x0AC)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x0B0, ISP_RD32((void *)(ISP_ADDR + 0x0B0)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x0B4, ISP_RD32((void *)(ISP_ADDR + 0x0B4)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x0B8, ISP_RD32((void *)(ISP_ADDR + 0x0B8)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x0BC, ISP_RD32((void *)(ISP_ADDR + 0x0BC)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x0E8, ISP_RD32((void *)(ISP_ADDR + 0x0E8)));	/* cq0 cur base */
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x0F4, ISP_RD32((void *)(ISP_ADDR + 0x0F4)));	/* fbc imgo */
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x0F8, ISP_RD32((void *)(ISP_ADDR + 0x0F8)));	/* fbc img2o */
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x110, ISP_RD32((void *)(ISP_ADDR + 0x110)));	/* mdp crop x */
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x114, ISP_RD32((void *)(ISP_ADDR + 0x114)));	/* mdp crop y */
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x138, ISP_RD32((void *)(ISP_ADDR + 0x138)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x13C, ISP_RD32((void *)(ISP_ADDR + 0x13C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x144, ISP_RD32((void *)(ISP_ADDR + 0x144)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x148, ISP_RD32((void *)(ISP_ADDR + 0x148)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x14C, ISP_RD32((void *)(ISP_ADDR + 0x14C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x204, ISP_RD32((void *)(ISP_ADDR + 0x204)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x20C, ISP_RD32((void *)(ISP_ADDR + 0x20C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x22C, ISP_RD32((void *)(ISP_ADDR + 0x22C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x230, ISP_RD32((void *)(ISP_ADDR + 0x230)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x234, ISP_RD32((void *)(ISP_ADDR + 0x234)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x238, ISP_RD32((void *)(ISP_ADDR + 0x238)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x23C, ISP_RD32((void *)(ISP_ADDR + 0x23C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x240, ISP_RD32((void *)(ISP_ADDR + 0x240)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x244, ISP_RD32((void *)(ISP_ADDR + 0x244)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x258, ISP_RD32((void *)(ISP_ADDR + 0x258)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x260, ISP_RD32((void *)(ISP_ADDR + 0x260)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x26C, ISP_RD32((void *)(ISP_ADDR + 0x26C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x270, ISP_RD32((void *)(ISP_ADDR + 0x270)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x27C, ISP_RD32((void *)(ISP_ADDR + 0x27C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x298, ISP_RD32((void *)(ISP_ADDR + 0x298)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x2B4, ISP_RD32((void *)(ISP_ADDR + 0x2B4)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x2D0, ISP_RD32((void *)(ISP_ADDR + 0x2D0)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x2D4, ISP_RD32((void *)(ISP_ADDR + 0x2D4)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x2F0, ISP_RD32((void *)(ISP_ADDR + 0x2F0)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x2F4, ISP_RD32((void *)(ISP_ADDR + 0x2F4)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x300, ISP_RD32((void *)(ISP_ADDR + 0x300)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x304, ISP_RD32((void *)(ISP_ADDR + 0x304)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x308, ISP_RD32((void *)(ISP_ADDR + 0x308)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x30C, ISP_RD32((void *)(ISP_ADDR + 0x30C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x310, ISP_RD32((void *)(ISP_ADDR + 0x310)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x314, ISP_RD32((void *)(ISP_ADDR + 0x314)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x318, ISP_RD32((void *)(ISP_ADDR + 0x318)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x31C, ISP_RD32((void *)(ISP_ADDR + 0x31C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x320, ISP_RD32((void *)(ISP_ADDR + 0x320)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x324, ISP_RD32((void *)(ISP_ADDR + 0x324)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x328, ISP_RD32((void *)(ISP_ADDR + 0x328)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x32C, ISP_RD32((void *)(ISP_ADDR + 0x32C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x330, ISP_RD32((void *)(ISP_ADDR + 0x330)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x334, ISP_RD32((void *)(ISP_ADDR + 0x334)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x338, ISP_RD32((void *)(ISP_ADDR + 0x338)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x33C, ISP_RD32((void *)(ISP_ADDR + 0x33C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x350, ISP_RD32((void *)(ISP_ADDR + 0x350)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x37C, ISP_RD32((void *)(ISP_ADDR + 0x37C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x398, ISP_RD32((void *)(ISP_ADDR + 0x398)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x410, ISP_RD32((void *)(ISP_ADDR + 0x410)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x414, ISP_RD32((void *)(ISP_ADDR + 0x414)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x418, ISP_RD32((void *)(ISP_ADDR + 0x418)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x41C, ISP_RD32((void *)(ISP_ADDR + 0x41C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x420, ISP_RD32((void *)(ISP_ADDR + 0x420)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x448, ISP_RD32((void *)(ISP_ADDR + 0x448)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4B0, ISP_RD32((void *)(ISP_ADDR + 0x4B0)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4B4, ISP_RD32((void *)(ISP_ADDR + 0x4B4)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4B8, ISP_RD32((void *)(ISP_ADDR + 0x4B8)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4BC, ISP_RD32((void *)(ISP_ADDR + 0x4BC)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4E8, ISP_RD32((void *)(ISP_ADDR + 0x4E8)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x534, ISP_RD32((void *)(ISP_ADDR + 0x534)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x538, ISP_RD32((void *)(ISP_ADDR + 0x538)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x53C, ISP_RD32((void *)(ISP_ADDR + 0x53C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x580, ISP_RD32((void *)(ISP_ADDR + 0x580)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x584, ISP_RD32((void *)(ISP_ADDR + 0x584)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x800, ISP_RD32((void *)(ISP_ADDR + 0x800)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x880, ISP_RD32((void *)(ISP_ADDR + 0x880)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x884, ISP_RD32((void *)(ISP_ADDR + 0x884)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x888, ISP_RD32((void *)(ISP_ADDR + 0x888)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x8A0, ISP_RD32((void *)(ISP_ADDR + 0x8A0)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x920, ISP_RD32((void *)(ISP_ADDR + 0x920)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x924, ISP_RD32((void *)(ISP_ADDR + 0x924)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x928, ISP_RD32((void *)(ISP_ADDR + 0x928)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x92C, ISP_RD32((void *)(ISP_ADDR + 0x92C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x930, ISP_RD32((void *)(ISP_ADDR + 0x930)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x934, ISP_RD32((void *)(ISP_ADDR + 0x934)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x938, ISP_RD32((void *)(ISP_ADDR + 0x938)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x93C, ISP_RD32((void *)(ISP_ADDR + 0x93C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x948, ISP_RD32((void *)(ISP_ADDR + 0x948)));	/* unp offset */
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x960, ISP_RD32((void *)(ISP_ADDR + 0x960)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x9C4, ISP_RD32((void *)(ISP_ADDR + 0x9C4)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x9E4, ISP_RD32((void *)(ISP_ADDR + 0x9E4)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x9E8, ISP_RD32((void *)(ISP_ADDR + 0x9E8)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x9EC, ISP_RD32((void *)(ISP_ADDR + 0x9EC)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xA00, ISP_RD32((void *)(ISP_ADDR + 0xA00)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xA04, ISP_RD32((void *)(ISP_ADDR + 0xA04)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xA08, ISP_RD32((void *)(ISP_ADDR + 0xA08)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xA0C, ISP_RD32((void *)(ISP_ADDR + 0xA0C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xA10, ISP_RD32((void *)(ISP_ADDR + 0xA10)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xA14, ISP_RD32((void *)(ISP_ADDR + 0xA14)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xA20, ISP_RD32((void *)(ISP_ADDR + 0xA20)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xAA0, ISP_RD32((void *)(ISP_ADDR + 0xAA0)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xACC, ISP_RD32((void *)(ISP_ADDR + 0xACC)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xB00, ISP_RD32((void *)(ISP_ADDR + 0xB00)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xB04, ISP_RD32((void *)(ISP_ADDR + 0xB04)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xB08, ISP_RD32((void *)(ISP_ADDR + 0xB08)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xB0C, ISP_RD32((void *)(ISP_ADDR + 0xB0C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xB10, ISP_RD32((void *)(ISP_ADDR + 0xB10)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xB14, ISP_RD32((void *)(ISP_ADDR + 0xB14)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xB18, ISP_RD32((void *)(ISP_ADDR + 0xB18)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xB1C, ISP_RD32((void *)(ISP_ADDR + 0xB1C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xB20, ISP_RD32((void *)(ISP_ADDR + 0xB20)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xB44, ISP_RD32((void *)(ISP_ADDR + 0xB44)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xB48, ISP_RD32((void *)(ISP_ADDR + 0xB48)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xB4C, ISP_RD32((void *)(ISP_ADDR + 0xB4C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xB50, ISP_RD32((void *)(ISP_ADDR + 0xB50)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xB54, ISP_RD32((void *)(ISP_ADDR + 0xB54)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xB58, ISP_RD32((void *)(ISP_ADDR + 0xB58)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xB5C, ISP_RD32((void *)(ISP_ADDR + 0xB5C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xB60, ISP_RD32((void *)(ISP_ADDR + 0xB60)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xBA0, ISP_RD32((void *)(ISP_ADDR + 0xBA0)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xBA4, ISP_RD32((void *)(ISP_ADDR + 0xBA4)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xBA8, ISP_RD32((void *)(ISP_ADDR + 0xBA8)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xBAC, ISP_RD32((void *)(ISP_ADDR + 0xBAC)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xBB0, ISP_RD32((void *)(ISP_ADDR + 0xBB0)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xBB4, ISP_RD32((void *)(ISP_ADDR + 0xBB4)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xBB8, ISP_RD32((void *)(ISP_ADDR + 0xBB8)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xBBC, ISP_RD32((void *)(ISP_ADDR + 0xBBC)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xBC0, ISP_RD32((void *)(ISP_ADDR + 0xBC0)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xC20, ISP_RD32((void *)(ISP_ADDR + 0xC20)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xCC0, ISP_RD32((void *)(ISP_ADDR + 0xCC0)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xCE4, ISP_RD32((void *)(ISP_ADDR + 0xCE4)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xCE8, ISP_RD32((void *)(ISP_ADDR + 0xCE8)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xCEC, ISP_RD32((void *)(ISP_ADDR + 0xCEC)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xCF0, ISP_RD32((void *)(ISP_ADDR + 0xCF0)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xCF4, ISP_RD32((void *)(ISP_ADDR + 0xCF4)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xCF8, ISP_RD32((void *)(ISP_ADDR + 0xCF8)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xCFC, ISP_RD32((void *)(ISP_ADDR + 0xCFC)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xD24, ISP_RD32((void *)(ISP_ADDR + 0xD24)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xD28, ISP_RD32((void *)(ISP_ADDR + 0xD28)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xD2C, ISP_RD32((void *)(ISP_ADDR + 0xD2c)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xD40, ISP_RD32((void *)(ISP_ADDR + 0xD40)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xD64, ISP_RD32((void *)(ISP_ADDR + 0xD64)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xD68, ISP_RD32((void *)(ISP_ADDR + 0xD68)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xD6C, ISP_RD32((void *)(ISP_ADDR + 0xD6c)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xD70, ISP_RD32((void *)(ISP_ADDR + 0xD70)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xD74, ISP_RD32((void *)(ISP_ADDR + 0xD74)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xD78, ISP_RD32((void *)(ISP_ADDR + 0xD78)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xD7C, ISP_RD32((void *)(ISP_ADDR + 0xD7C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xDA4, ISP_RD32((void *)(ISP_ADDR + 0xDA4)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xDA8, ISP_RD32((void *)(ISP_ADDR + 0xDA8)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0xDAC, ISP_RD32((void *)(ISP_ADDR + 0xDAC)));
#if 0
	ISP_WR32((void *)(ISP_ADDR + 0x4618), 0x0000FFFF);
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4010, ISP_RD32((void *)(ISP_ADDR + 0x4010)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4014, ISP_RD32((void *)(ISP_ADDR + 0x4014)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4018, ISP_RD32((void *)(ISP_ADDR + 0x4018)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x401C, ISP_RD32((void *)(ISP_ADDR + 0x401C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4020, ISP_RD32((void *)(ISP_ADDR + 0x4020)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4024, ISP_RD32((void *)(ISP_ADDR + 0x4024)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4028, ISP_RD32((void *)(ISP_ADDR + 0x4028)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x402C, ISP_RD32((void *)(ISP_ADDR + 0x402C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4100, ISP_RD32((void *)(ISP_ADDR + 0x4100)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4104, ISP_RD32((void *)(ISP_ADDR + 0x4104)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4108, ISP_RD32((void *)(ISP_ADDR + 0x4108)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x410C, ISP_RD32((void *)(ISP_ADDR + 0x410C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4120, ISP_RD32((void *)(ISP_ADDR + 0x4120)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x412C, ISP_RD32((void *)(ISP_ADDR + 0x412C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x420C, ISP_RD32((void *)(ISP_ADDR + 0x420C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4234, ISP_RD32((void *)(ISP_ADDR + 0x4234)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4240, ISP_RD32((void *)(ISP_ADDR + 0x4240)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4300, ISP_RD32((void *)(ISP_ADDR + 0x4300)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4304, ISP_RD32((void *)(ISP_ADDR + 0x4304)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4600, ISP_RD32((void *)(ISP_ADDR + 0x4600)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4604, ISP_RD32((void *)(ISP_ADDR + 0x4604)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4608, ISP_RD32((void *)(ISP_ADDR + 0x4608)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x460C, ISP_RD32((void *)(ISP_ADDR + 0x460C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4610, ISP_RD32((void *)(ISP_ADDR + 0x4610)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4614, ISP_RD32((void *)(ISP_ADDR + 0x4614)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4618, ISP_RD32((void *)(ISP_ADDR + 0x4618)));
	ISP_WR32((void *)(ISP_ADDR + 0x461C), 0x10);
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x461C, ISP_RD32((void *)(ISP_ADDR + 0x461C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4620, ISP_RD32((void *)(ISP_ADDR + 0x4620)));
	...
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4618, ISP_RD32((void *)(ISP_ADDR + 0x4618)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4624, ISP_RD32((void *)(ISP_ADDR + 0x4624)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4628, ISP_RD32((void *)(ISP_ADDR + 0x4628)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x462C, ISP_RD32((void *)(ISP_ADDR + 0x462C)));
	LOG_ERR("0x%08X %08X", ISP_TPIPE_ADDR + 0x4630, ISP_RD32((void *)(ISP_ADDR + 0x4630)));
#endif
	...
	for (i = 0x44; i <= 0x48; i += 4)
		LOG_ERR("0x%08X %08X ", (unsigned int)(ISP_TPIPE_ADDR + i), ISP_RD32(ISP_ADDR + i));
	...
	LOG_ERR("0x%08X %08X ", (unsigned int)(ISP_ADDR_CAMINF), ISP_RD32((void *)ISP_ADDR_CAMINF));
	...
	LOG_ERR("0x%08X %08X ", ISP_ADDR + 0x160, ISP_RD32(ISP_ADDR + 0x160));
	LOG_ERR("0x%08X %08X ", ISP_ADDR + 0x164, ISP_RD32(ISP_ADDR + 0x164));
...
*Rdy should be 1    at idle or end of tile, if not 0, mdp isn't ready
...
static inline void Prepare_Enable_ccf_clock(void)
{
	...
	/*
	  ...
	  * < 0 : do power on fail
	 */
	/*LOG_DBG("ISP power/clock on by PM ==>"); */
	ret = pm_runtime_get_sync(g_pmdev_disp);
	if (ret < 0)
		LOG_ERR("cannot pm_runtime_get_sync(DISP)\n");
	...
	for (i = 0; i < ARRAY_SIZE(ispclk); i++) {
		...
		if (ret)
			LOG_ERR("cannot prepare_enable %s clock\n", ispclk[i].clkname);
	}
}
...
static MVOID ISP_EnableClock(MBOOL En, MUINT32 LogLevel)
{
	...
	if (En) {		/* enable clock. */
		...
		/*
		  * LOG_DBG("[MTCMOS CHECK]===> [0x1000660c]=0x%x , [0x10006610]=0x%x ",
		  * ISP_RD32(g_scpsys_baseaddr+0x60c), ISP_RD32(g_scpsys_baseaddr+0x610));

		  * if ((ISP_RD32(g_scpsys_baseaddr+0x60c)&0x28) != 0x28)
		  * LOG_ERR("MTCMOS: DISP-Bit3(%d), ISP-Bit5(%d) is not opened !!",
		  * (ISP_RD32(g_scpsys_baseaddr+0x60c)&0x08), (ISP_RD32(g_scpsys_baseaddr+0x60c)&0x20));

		  * LOG_DBG("[VENCPLL CHECK]===> [0x1000F800]=0x%x , [0x1000F80c]=0x%x ",
		  * ISP_RD32(g_ddrphy_baseaddr+0x800), ISP_RD32(g_ddrphy_baseaddr+0x80c));
		  * LOG_DBG("[IMG_CG_CON CHECK]===> [0x%lx]=0x%x", ISP_IMGSYS_BASE, ISP_RD32(ISP_IMGSYS_BASE));
		 */
		/*LOG_DBG("(0x%08x 0x%08x)", ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_SET),
		  * ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_PORT));
		  */
...
/* LOG_DBG("CQ STATUS RETURN:>"); */
			...
			if (LogLevel > 0) {
				LOG_DBG("(0x%08x 0x%08x)",
					ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_SET),
					ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_PORT));
			}
/* LOG_DBG(" <"); */
		}
...
static MINT32 ISP_ReadReg(ISP_REG_IO_STRUCT *pRegIo)
{
	...
	for (i = 0; i < pRegIo->Count; i++) {
		if (get_user(reg.Addr, &(pData->Addr)) != 0) {
		...
		if (put_user(reg.Val, &(pData->Val)) != 0) {
...
static MINT32 ISP_WriteReg(ISP_REG_IO_STRUCT *pRegIo)
{
	...
	if (atomic_read(&(g_IspInfo.HoldInfo.HoldEnable))) {
		...
		pData =
		    kmalloc((pRegIo->Count) * sizeof(ISP_REG_STRUCT),
					       GFP_ATOMIC);
		if (pData == NULL) {
...
static long ISP_REF_CNT_CTRL_FUNC(unsigned long Param)
{
	...
	if (copy_from_user(&ref_cnt_ctrl, (void __user *)Param, sizeof(ISP_REF_CNT_CTRL_STRUCT)) ==
		...
		if (ref_cnt_ctrl.id < ISP_REF_CNT_ID_MAX) {
			...
			if ((imem_ref_cnt == 0) && (ref_cnt_ctrl.ctrl == ISP_REF_CNT_DEC_AND_RESET_IF_LAST_ONE)) {
				ISP_Reset();
...
static MINT32 ISP_RTBC_ENQUE(MINT32 dma)
{
	...
	if (buffer_exist) {
		if (g_pstRTBuf->ring_buf[rt_dma].data[i].bFilled != ISP_RTBC_BUF_EMPTY) {
	...
	if (g_pstRTBuf->ring_buf[rt_dma].empty_count == 1) {
		if (_imgo_ == rt_dma) {
...
static MINT32 ISP_RTBC_DEQUE(MINT32 dma)
{
	...
	for (i = 0; i < g_pstRTBuf->ring_buf[rt_dma].total_count; i++) {
		...
		if (g_pstRTBuf->ring_buf[rt_dma].data[index].bFilled == ISP_RTBC_BUF_FILLED) {
			...
			if (copy_from_user
				...
				if ((reg_val & 0x01) || (reg_val2 & 0x01)) {
					...
					Timeout =
					    ...
					    (g_IspInfo.WaitQueueHead, (g_EnqBuf == 0),
		...
		case ISP_RT_BUF_CTRL_DEQUE:
			...
			if ((reg_val & 0x01) || (reg_val2 & 0x01)) {
				...
				for (i = 0; i < g_deque_buf.count; i++) {
					...
					if (g_IspInfo.DebugMask & ISP_DBG_BUF_CTRL) {
						LOG_DBG
			("[rtbc][DEQ]:T\"%d.%06d\",co(%08lx),c2o(%08lx),i(%d),iBuf+i(%d),V(%llx),P(%x),O(%x),2O(%x)",
					     g_deque_buf.data[i].timeStampS,
					     g_deque_buf.data[i].timeStampUs,
					     imgo_fbc.Reg_val, img2o_fbc.Reg_val, i,
					     iBuf + i, g_deque_buf.data[i].base_vAddr,
					     g_deque_buf.data[i].base_pAddr,
					     ISP_RD32((void *)ISP_REG_ADDR_IMGO_BASE_ADDR),
					     ISP_RD32((void *)ISP_REG_ADDR_IMG2O_BASE_ADDR));
					}
...
#if 0
				LOG_DBG
				    ("+LARB in DEQUE,BWL(0x%08X)/(0x%08X)/(0x%08X)/(0x%08X)",
				     ISP_RD32(0xF0202000 + 0x204), ISP_RD32(0xF0202000 + 0x20c),
				     ISP_RD32(0xF0202000 + 0x210), ISP_RD32(0xF0202000 + 0x214));
				LOG_DBG("220(0x%08X)/(0x%08X),0x14(0x%08X)/(0x%08X)/(0x%08X)/(0x%08X)/(0x%08X)",
...
static MINT32 ISP_SOF_Buf_Get(unsigned long long sec, unsigned long usec)
{
	...
	if (g_pstRTBuf->dropCnt == 0) {
		...
		if (g_pstRTBuf->ring_buf[_imgo_].total_count > ISP_RT_CQ0C_BUF_SIZE) {
		...
		if (g_IspInfo.DebugMask & ISP_DBG_BUF_CTRL) {
			LOG_DBG
			    ("[rtbc]TStamp(%d.%06d),curr(%d),pa(0x%x/0x%x)",
			...
			LOG_DBG("o2o(0x%x/0x%x),fbc_o(0x%08x),fbc_2o(0x%08x),cq0c(0x%x)",
				 ISP_RD32((void *)ISP_REG_ADDR_IMGO_BASE_ADDR),
			     ISP_RD32((void *)ISP_REG_ADDR_IMG2O_BASE_ADDR),
	...
	for (i = 0; i <= 1; i++) {
		...
		if (g_pstRTBuf->ring_buf[i_dma].empty_count == 0) {
			...
			if (g_pstRTBuf->ring_buf[i_dma].empty_count >= 2) {
				/* next buffer */
				next =
				    (g_pstRTBuf->ring_buf[i_dma].start +
...
static MINT32 ISP_DONE_Buf_Time(unsigned long long sec, unsigned long usec)
{
	...
	for (i = 0; i <= 1; i++) {
		...
		/* once if buffer put into queue between SOF and ISP_DONE. */
		if (g_pstRTBuf->ring_buf[i_dma].active == MFALSE) {
			if (g_IspInfo.DebugMask & ISP_DBG_INT)
	...
	while (1) {
		IrqStatus[ISP_IRQ_TYPE_INT] =
		    (ISP_RD32((void *)ISP_REG_ADDR_INT_STATUS) &
		     (g_IspInfo.IrqInfo.Mask[ISP_IRQ_TYPE_INT] | g_IspInfo.
		      IrqInfo.ErrMask[ISP_IRQ_TYPE_INT]));
		IrqStatus[ISP_IRQ_TYPE_DMA] =
		    (ISP_RD32((void *)ISP_REG_ADDR_DMA_INT) &
		     (g_IspInfo.IrqInfo.Mask[ISP_IRQ_TYPE_DMA] | g_IspInfo.
		      IrqInfo.ErrMask[ISP_IRQ_TYPE_DMA]));
		IrqStatus[ISP_IRQ_TYPE_INTB] =
		    (ISP_RD32((void *)ISP_REG_ADDR_INTB_STATUS) &
		     (g_IspInfo.IrqInfo.Mask[ISP_IRQ_TYPE_INTB] | g_IspInfo.
		      IrqInfo.ErrMask[ISP_IRQ_TYPE_INTB]));
		IrqStatus[ISP_IRQ_TYPE_DMAB] =
		    (ISP_RD32((void *)ISP_REG_ADDR_DMAB_INT) &
		     (g_IspInfo.IrqInfo.Mask[ISP_IRQ_TYPE_DMAB] | g_IspInfo.
		      IrqInfo.ErrMask[ISP_IRQ_TYPE_DMAB]));
		IrqStatus[ISP_IRQ_TYPE_INTC] =
		    (ISP_RD32((void *)ISP_REG_ADDR_INTC_STATUS) &
		     (g_IspInfo.IrqInfo.Mask[ISP_IRQ_TYPE_INTC] | g_IspInfo.
		      IrqInfo.ErrMask[ISP_IRQ_TYPE_INTC]));
		IrqStatus[ISP_IRQ_TYPE_DMAC] =
		    (ISP_RD32((void *)ISP_REG_ADDR_DMAC_INT) &
		     (g_IspInfo.IrqInfo.Mask[ISP_IRQ_TYPE_DMAC] | g_IspInfo.
		      IrqInfo.ErrMask[ISP_IRQ_TYPE_DMAC]));
		IrqStatus[ISP_IRQ_TYPE_INTX] =
		    (ISP_RD32((void *)ISP_REG_ADDR_INT_STATUSX) &
		     (g_IspInfo.IrqInfo.Mask[ISP_IRQ_TYPE_INTX] | g_IspInfo.
		      IrqInfo.ErrMask[ISP_IRQ_TYPE_INTX]));
		IrqStatus[ISP_IRQ_TYPE_DMAX] =
		    (ISP_RD32((void *)ISP_REG_ADDR_DMA_INTX) &
		     (g_IspInfo.IrqInfo.Mask[ISP_IRQ_TYPE_DMAX] | g_IspInfo.
		      IrqInfo.ErrMask[ISP_IRQ_TYPE_DMAX]));

		for (i = 0; i < ISP_IRQ_TYPE_AMOUNT; i++) {
			if (g_IspInfo.IrqInfo.ErrMask[i] & IrqStatus[i]) {
				/* LOG_ERR("Error IRQ, Type(%d), Status(0x%08X)",
				  *i,g_IspInfo.IrqInfo.ErrMask[i] & IrqStatus[i]);
				  */
			...
			if (g_IspInfo.DebugMask & ISP_DBG_INT) {
				...
				/* LOG_DBG("Mask(0x%08X), ErrMask(0x%08X)",
				  * g_IspInfo.IrqInfo.Mask[i], g_IspInfo.IrqInfo.ErrMask[i]);
				  */
			}
	...
	for (i = 0; i < 12; i++) {
		...
		if (err_ctrl & (*pErr)) {
			...
			LOG_DBG("(0x%08x,0x%08x),dbg(0x%08x, 0x%08x)", addr,
				ISP_RD32((unsigned long)addr), ISP_RD32(0xF5004160),
				ISP_RD32(0xF5004164));
		}
#else
		addr = pErr[1];
...
static __tcmfunc irqreturn_t ISP_Irq(MINT32 Irq, MVOID *DeviceId)
{
	...
	if (!CAM_HAL_VER_IS3) {
			...
			if (g_IspInfo.IrqInfo.ErrMask[i] & IrqStatus[i]) {
				...
				if (i != ISP_IRQ_TYPE_INTX
					...
					if (i != ISP_IRQ_TYPE_INTX) {	/* to reduce isp log */
						LOG_DBG
						    ("Error IRQ, Type(%d), Status(0x%08X),m:%x, irq:%x",
						     i, g_IspInfo.IrqInfo.ErrMask[i] & IrqStatus[i],
						     g_IspInfo.IrqInfo.ErrMask[i], IrqStatus[i]);
					}
					/* TODO: Add error handler... */
				...
				if (i != ISP_IRQ_TYPE_INTX && (g_IspInfo.IrqInfo.ErrMask[i]
		 ...
		 *
		 *  #if defined(_debug_dma_err_)
		 *  LOG_ERR("[rtbc]StatusX(0x%08X), DMA_ERR",IrqStatus[ISP_IRQ_TYPE_INTX]);
		 *
		 *  //allan
		 *  DMAErrHandler();
		 *
		...
		/* LOG_ERR("[rtbc]dropframe st(0x%08X),co(0x%08X),c2o(0x%08X)",IrqStatus_fbc_int, */
		/* ISP_RD32(ISP_REG_ADDR_IMGO_FBC),ISP_RD32(ISP_REG_ADDR_IMG2O_FBC)); */
		/* } */
		...
		if (IrqStatus[ISP_IRQ_TYPE_INT] & ISP_IRQ_INT_STATUS_PASS1_TG1_DON_ST) {
			...
			if (g_newImg2oAddr != DEFAULT_PA &&
			    ...
			    ((img2o_fbc.Bits.fbc_cnt == img2o_fbc.Bits.fb_num)
		...
		if (IrqStatus[ISP_IRQ_TYPE_INT] & ISP_IRQ_INT_STATUS_SOF1_INT_ST) {
			...
			if ((sof_pa != 0) && (sof_pa == g_oldImgoAddr)
			    ...
			    && ((imgo_fbc.Bits.fbc_cnt == imgo_fbc.Bits.fb_num)
				|| (imgo_fbc.Bits.fbc_cnt == (imgo_fbc.Bits.fb_num - 1)))) {
			...
			if (g_sof_pass1done == 1) {
				LOG_DBG("P1_SOF_%d: Lost pass1 done",
					((ISP_RD32(ISP_REG_ADDR_TG_INTER_ST) & 0x00FF0000) >> 16));
			}
			g_sof_pass1done = 1;
		}
		...
		for (j = 0; j < ISP_IRQ_TYPE_AMOUNT_FRMB; j++) {
			for (i = 0; i < IRQ_USER_NUM_MAX; i++) {
				...
				if (IspInfo_FrmB.IrqInfo.MarkedFlag[i][j] & IspInfo_FrmB.
				    IrqInfo.Mask[j]) {
					for (k = 0; k < 32; k++) {
					if ((IrqStatus[j] & IspInfo_FrmB.
					 IrqInfo.Mask[j]) & (1 << k)) {
						idx = my_get_pow_idx(1 << k);
						IspInfo_FrmB.
						    IrqInfo.LastestSigTime_usec[j][idx] =
						    (unsigned int)(time_frmb_usec);
						IspInfo_FrmB.
						    IrqInfo.LastestSigTime_sec[j][idx] =
						    (unsigned int)(time_frmb_sec);
						IspInfo_FrmB.
						    IrqInfo.PassedBySigCnt[i][j][k]++;
					}
					}
...
static long ISP_ioctl(struct file *pFile, MUINT32 Cmd, unsigned long Param)
{
	...
	case ISP_USER_PRINT_CTRL:
		{
			if (copy_from_user
			    (&UserLog, (void __user *)Param, sizeof(ISP_DEBUG_PRINT_STRUCT)) == 0) {
	...
	case ISP_SENSOR_FREQ_CTRL:
		{
#if 0				/* Old method, already move setting pclk to seninf->kd_sensorlist.c: setMCLK() */
			MUINT32 senFreq = 0;
...
static MINT32 ISP_open(struct inode *pInode, struct file *pFile)
{
	...
	g_IspInfo.BufInfo.Read.pData = kmalloc(ISP_BUF_SIZE, GFP_ATOMIC);
...
static MINT32 ISP_release(struct inode *pInode, struct file *pFile)
{
	...
	if (g_IspInfo.UserCount > 0) {
		spin_unlock(&(g_IspInfo.SpinLockIspRef));
		LOG_DBG("+ Curr UserCount(%d->%d), (process, pid, tgid)=(%s, %d, %d), users exist",
			(g_IspInfo.UserCount + 1), g_IspInfo.UserCount, current->comm, current->pid,
			current->tgid);
		goto EXIT;
	} else {
		spin_unlock(&(g_IspInfo.SpinLockIspRef));
...
EXIT:
	 ...
	 */
	ISP_EnableClock(MFALSE, 1);
	LOG_DBG("-,UserCount(%d), g_EnableClkCnt(%d)", g_IspInfo.UserCount, g_EnableClkCnt);
	return 0;
}
...
static unsigned long of_get_nodeVA(const char *comp_str)
{
	...
	if (dev_node == NULL) {
		LOG_ERR("of_find_compatible_node(%s) fail", comp_str);
	} else {
		LOG_DBG("of_find_compatible_node(%s) success !!", comp_str);

		plat_dev = of_find_device_by_node(dev_node);
		if (WARN_ON(!plat_dev))
			LOG_ERR("plat_dev NULL");

		LOG_DBG("of_find_device_by_node(dev_node) success !!");

		ret_baseaddr = (unsigned long)of_iomap(plat_dev->dev.of_node, 0);
		LOG_DBG("(%s)ret_baseaddr PA=0x%lx, VA = 0x%lx",
			comp_str, of_getISPPA(plat_dev->dev.of_node, 0), ret_baseaddr);
	}
	return ret_baseaddr;
}
...
static MINT32 ISP_probe(struct platform_device *pDev)
{
	...
	for (i = 0; i < ISP_CAM_BASEADDR_NUM; i++) {
		 ...
		 *   i, pDev->dev.of_node->name, gISPSYS_Reg[i], gISPSYS_Reg_FrmB[i]);
		  */
		LOG_INF("DT, i=%d, PA(0x%lx), map_VA=0x%lx/0x%lx\n",
			i, of_getISPPA(pDev->dev.of_node, i), gISPSYS_Reg[i], gISPSYS_Reg_FrmB[i]);
	}

	/* Save MTCMOS/Clock status reg addr for checking */
	g_scpsys_baseaddr = of_get_nodeVA("mediatek,mt8167-scpsys");
	g_ddrphy_baseaddr = of_get_nodeVA("mediatek,mt8167-ddrphy");
	...
	for (i = 0; i < ISP_CAM_IRQ_IDX_NUM; i++) {
		...
		if (i == ISP_CAM0_IRQ_IDX)
			Ret = request_irq(cam_isp_dev->irq[i], (irq_handler_t) ISP_Irq, IRQF_TRIGGER_NONE, "ISP", NULL);
		...
		LOG_INF("DT, i=%d, name=%s, map_irq=%d , get_hardware_irq(%d)\n", i,
			pDev->dev.of_node->name, cam_isp_dev->irq[i],
			get_hardware_irq(cam_isp_dev->irq[i]));
...
/* [Houston] Need to power on ISP firstly to avoid ISP reg UN-accessed +++ */
	...
	WARN_ON(IS_ERR(g_pmdev_isp));
	pm_runtime_enable(g_pmdev_isp);
	pm_runtime_get_sync(g_pmdev_isp);
	LOG_INF("[Houston] pm_runtime_get_sync(ISP)");
/* [Houston] --- */
#endif
...
*
********************************************************************************/
static ssize_t ISP_RegDebug(struct file *pFile, const char *pBuffer, size_t Count, loff_t *pData)
{
	char RegBuf[64] = { '\0' };
	...
	if (sscanf(RegBuf, "debugmask=%x", &Addr) == 1) {
		...
		if ((ISP_ADDR_CAMINF + Addr >= ISP_ADDR) &&
			...
			LOG_ERR("Wrong address out of range (0x%x)",
				(unsigned int)(ISP_ADDR_CAMINF + Addr));
		}
	} else if (kstrtouint(RegBuf, 16, &Addr) == 1) {
		if ((ISP_ADDR_CAMINF + Addr >= ISP_ADDR) &&
		    (ISP_ADDR_CAMINF + Addr < (ISP_ADDR_CAMINF + ISP_REG_RANGE))) {
