...
/* HDMI interrupt use flag, when TRUE,it means use the hdmi shell ,or it use the CEC interrupt */
unsigned int HDMI_INTERRUPT_FLAG = TRUE;
...
{
	if (HDMI_INTERRUPT_FLAG)
		return (bReadByteHdmiGRL(GRL_STATUS) & STATUS_HTPLG);
	else
...
{
	if (HDMI_INTERRUPT_FLAG)
		return (bReadByteHdmiGRL(GRL_STATUS) & STATUS_PORD);
	else
	...
	if (IsCECStatus(STATE_TXING_FRAME)) {
		if (IsCECStatus(STATE_HW_RETX)) {
			...
			if (IS_TX_FSM_IDLE() && IS_TX_FINISH()) {
				...
				ClrCECStatus(STATE_TXING_FRAME);
				HDMI_CEC_LOG
				    ("TX is COMPLETED with: H ACK: %x and D ACK %x\n",
...
static void _CEC_tx_msg_notify(__u8 result, void *tag)
{
	if (result == 0x00) {
...
void hdmi_cec_api_get_txsts(struct CEC_ACK_INFO_T *pt)
{
	memcpy(pt, &cec_send_result, sizeof(struct CEC_ACK_INFO_T));
}
...
static void _CEC_TX_Queue_Loop(void)
{
	...
	if (IsCECStatus(STATE_TXING_FRAME)) {
		if (IsCECErrorFlag(ERR_TX_MISALARM) && (TX_FAIL_RECORD() != 0)
		    && IS_TX_FSM_FAIL()) {
			DISABLE_ALL_TX_INT();
...
unsigned char _CEC_TX_Enqueue(struct CEC_FRAME_DESCRIPTION_IO *frame)
{
	...
	if (frame->size == 1) {
		HDMI_CEC_LOG("Polling LA = 0x%x\n",
			     (unsigned char)((frame->blocks.header.initiator << 4) |
					     frame->blocks.header.destination));
	} else
...
void hdmi_CECMWSetLA(struct CEC_DRV_ADDR_CFG_T *prAddr)
{
...
void hdmi_CECMWSend(struct CEC_SEND_MSG_T *msg)
{
	unsigned int i4Ret;
	unsigned char i;
	...
	if ((msg->t_frame_info.ui1_init_addr > 0xf)
	    || (msg->t_frame_info.ui1_dest_addr > 0xf)
