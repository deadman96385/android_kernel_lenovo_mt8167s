...
void hdmi_InfoframeSetting(unsigned char i1typemode, unsigned char i1typeselect)
{
	...
	if ((i1typemode == 0xff) && (i1typeselect == 0xff)) {
		pr_err("Arg1: Infoframe output type\n");
		pr_err("1: AVi, 2: Mpeg, 3: SPD\n");
		pr_err("4: Vendor, 5: Audio, 6: ACP\n");
		pr_err("7: ISRC1, 8: ISRC2,  9:GENERIC\n");
		pr_err("10:GAMUT\n");

		pr_err("Arg2: Infoframe data select\n");
		pr_err("0: old(default), 1: new;\n");
		return;
	}
