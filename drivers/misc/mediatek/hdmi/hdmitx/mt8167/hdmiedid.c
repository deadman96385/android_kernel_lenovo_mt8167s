...
#if defined(MHL_BRIDGE_SUPPORT)
const MHL_BRIDGE_DRIVER *mhl_bridge_drv;
#endif
		...
		if (b3D_Multi_present == 0x20) {
			...
			while (((15 - bLatency_offset +
				...
				b3D_MASK_7_0 =
					...
					/* support frame packet */
					if ((b3D_Structure_7_0 & 0x01) == 0x01) {
						u23D_MASK_ALL =	(((unsigned short)(b3D_MASK_15_8)) << 8)
...
static void  vParser_User_Extension_Tag(unsigned char *prData)
{	/* Use Extended Tag */
...
void vParserCEADataBlock(unsigned char *prData, unsigned char bLen)
{
	...
	while (bLen) {
		...
		if (bType == 0x02)
			...
			vParser_User_Extension_Tag(prData);
		else
			pr_err("vParserCEADataBlock: Invalid Data Block\n");
	...
	for (bIdx = 6; bIdx >= 1; bIdx--) {
		_HdmiSinkAvCap.ui1_sink_pcm_ch_sampling[bIdx - 1] |= _HdmiSinkAvCap.ui1_sink_pcm_ch_sampling[bIdx];
		_HdmiSinkAvCap.ui1_sink_dsd_ch_sampling[bIdx - 1] |= _HdmiSinkAvCap.ui1_sink_dsd_ch_sampling[bIdx];
	}
...
void hdmi_checkedid(unsigned char i1noedid)
{
	...
	ret = mhl_bridge_drv->getedid(&_bEdidData[0]);
	vSetSharedInfo(SI_EDID_EXT_BLOCK_NO, _bEdidData[EDID_ADDR_EXT_BLOCK_FLAG]);
	...
	if ((i4SharedInfo(SI_EDID_EXT_BLOCK_NO) * EDID_BLOCK_LEN) < EDID_SIZE)
		vParserExtEDIDState(&_bEdidData[0]);

	if (mhl_bridge_drv->getppmodesupport() == 0) {
		pr_err("[it668x]can not support pp mode\n");
		_HdmiSinkAvCap.ui4_sink_cea_ntsc_resolution &= ~SINK_1080P60;
		_HdmiSinkAvCap.ui4_sink_cea_pal_resolution &= ~SINK_1080P50;
		_HdmiSinkAvCap.ui4_sink_dtd_ntsc_resolution &= ~SINK_1080P60;
		_HdmiSinkAvCap.ui4_sink_dtd_pal_resolution &= ~SINK_1080P50;
		_HdmiSinkAvCap.ui4_sink_native_ntsc_resolution &= ~SINK_1080P60;
		_HdmiSinkAvCap.ui4_sink_native_pal_resolution &= ~SINK_1080P50;
	} else {
		pr_err("[it668x]support pp mode\n");
	}
...
void hdmi_AppGetEdidInfo(struct HDMI_EDID_INFO_T *pv_get_info)
{
	HDMI_EDID_FUNC();
