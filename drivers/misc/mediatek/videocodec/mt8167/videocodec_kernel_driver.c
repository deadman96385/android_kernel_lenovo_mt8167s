...
#ifdef CONFIG_OF
	MODULE_MFV_PR_DEBUG("vdec_power_on D+\n");
...
void vdec_power_off(void)
{
	...
	if (gu4VdecPWRCounter != 0)  {
		...
		/* Central power off */
#ifdef CONFIG_OF
	MODULE_MFV_PR_DEBUG("vdec_power_off D+\n");
...
#ifdef CONFIG_OF
	MODULE_MFV_PR_DEBUG("venc_power_on D+\n");
...
void venc_power_off(void)
{
	...
	if (gu4VencPWRCounter == 0) {
		MODULE_MFV_PR_DEBUG("venc_power_off none +\n");
	} else {
		gu4VencPWRCounter--;
		MODULE_MFV_PR_DEBUG("venc_power_off D+\n");
#ifdef CONFIG_OF
...
void dec_isr(void)
{
	...
	if ((u4CgStatus & 0x10) != 0) {
		MODULE_MFV_PR_ERR("[MFV][ERROR] DEC ISR, VDEC active is not 0x0 (0x%08x)", u4CgStatus);
		return;
	}

	u4DecDoneStatus = VDO_HW_READ(KVA_VDEC_MISC_BASE + 0xA4);
	if ((u4DecDoneStatus & (0x1 << 16)) != 0x10000) {
		MODULE_MFV_PR_ERR("[MFV][ERROR] DEC ISR, Decode done status is not 0x1 (0x%08x)",
			 u4DecDoneStatus);
		return;
	}
...
void enc_isr(void)
{
	...
	if (grVcodecEncHWLock.pvHandle == 0) {
		MODULE_MFV_PR_ERR("[ERROR] NO one Lock Enc HW, please check!!\n");
	...
	if (grVcodecEncHWLock.eDriverType == VAL_DRIVER_TYPE_H264_ENC) {	/*added by bin.liu  hardwire */
		...
		/* MODULE_MFV_PR_DEBUG("[enc_isr] VAL_DRIVER_TYPE_HEVC_ENC %d!!\n", gu4HwVencIrqStatus); */
		...
		MODULE_MFV_PR_ERR("enc_isr Invalid lock holder driver type = %d\n", grVcodecEncHWLock.eDriverType);
	...
	MODULE_MFV_PR_DEBUG("[MFV] enc_isr ISR set EncIsrEvent done\n");
}
...
static long vcodec_alloc_non_cache_buffer(unsigned long arg)
{
	...
	MODULE_MFV_PR_DEBUG("VCODEC_ALLOC_NON_CACHE_BUFFER + tid = %d\n", current->pid);

	user_data_addr = (VAL_UINT8_T *)arg;
	ret = copy_from_user(&rTempMem, user_data_addr, sizeof(VAL_MEMORY_T));
	if (ret) {
		MODULE_MFV_PR_ERR("[ERROR] VCODEC_ALLOC_NON_CACHE_BUFFER, copy_from_user failed: %lu\n", ret);
		return -EFAULT;
	}
	...
	if ((rTempMem.u4ReservedSize == 0) || (rTempMem.pvMemPa == 0)) {
		MODULE_MFV_PR_ERR("[ERROR] dma_alloc_coherent fail in VCODEC_ALLOC_NON_CACHE_BUFFER\n");
		  return -EFAULT;
	}

	MODULE_MFV_PR_DEBUG("[VCODEC] kernel va = 0x%lx, kernel pa = 0x%lx, memory size = %lu\n",
	...
	if (ret) {
		MODULE_MFV_PR_ERR("[ERROR] VCODEC_ALLOC_NON_CACHE_BUFFER, copy_to_user failed: %lu\n", ret);
		return -EFAULT;
	}
...
static long vcodec_free_non_cache_buffer(unsigned long arg)
{
	...
	MODULE_MFV_PR_DEBUG("VCODEC_FREE_NON_CACHE_BUFFER + tid = %d\n", current->pid);

	user_data_addr = (VAL_UINT8_T *)arg;
	ret = copy_from_user(&rTempMem, user_data_addr, sizeof(VAL_MEMORY_T));
	if (ret) {
		MODULE_MFV_PR_ERR("[ERROR] VCODEC_FREE_NON_CACHE_BUFFER, copy_from_user failed: %lu\n", ret);
		return -EFAULT;
	}
	...
	ret = copy_to_user(user_data_addr, &rTempMem, sizeof(VAL_MEMORY_T));
	if (ret) {
		MODULE_MFV_PR_ERR("[ERROR] VCODEC_FREE_NON_CACHE_BUFFER, copy_to_user failed: %lu\n", ret);
		return -EFAULT;
	}
			...
			if (grVcodecDecHWLock.pvHandle == 0) { /* No one holds dec hw lock now */
				...
				if (rHWLock.bSecureInst == VAL_FALSE) {
					if (request_irq(VDEC_IRQ_ID, (irq_handler_t) video_intr_dlr,
						...
						MODULE_MFV_PR_ERR("[VCODEC_DEBUG][ERROR] error to request dec irq\n");
					else
						MODULE_MFV_PR_DEBUG("[VCODEC_DEBUG] success to request dec irq\n");
				}
#else
...
static long vcodec_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case VCODEC_ALLOC_NON_CACHE_BUFFER:
	{
		...
		if (ret) {
			MODULE_MFV_PR_ERR("[ERROR] VCODEC_ALLOC_NON_CACHE_BUFFER failed! %lu\n", ret);
			return ret;
		}
	}
	break;
	...
	case VCODEC_FREE_NON_CACHE_BUFFER:
	{
		...
		if (ret) {
			MODULE_MFV_PR_ERR("[ERROR] VCODEC_FREE_NON_CACHE_BUFFER failed! %lu\n", ret);
			return ret;
		}
	}
	break;
	...
	case VCODEC_GET_CORE_LOADING:
	{
		...
		if (rTempCoreLoading.CPUid > num_possible_cpus()) {
			MODULE_MFV_PR_ERR("[ERROR] rTempCoreLoading.CPUid(%d) > num_possible_cpus(%d)\n",
			rTempCoreLoading.CPUid, num_possible_cpus());
			return -EFAULT;
		}
		if (rTempCoreLoading.CPUid < 0) {
			MODULE_MFV_PR_ERR("[ERROR] rTempCoreLoading.CPUid(%d) < 0\n", rTempCoreLoading.CPUid);
			return -EFAULT;
		}
...
{
	MODULE_MFV_PR_DEBUG("[VCODEC_DEBUG] vcodec_open\n");

	mutex_lock(&DriverOpenCountLock);
	MT8167Driver_Open_Count++;

	MODULE_MFV_PR_ERR("vcodec_open pid = %d, MT8167Driver_Open_Count %d\n", current->pid,
		 MT8167Driver_Open_Count);
...
static int venc_hw_reset(int type)
{
	...
	if (eValRet == VAL_RESULT_INVALID_ISR || gu4HwVencIrqStatus != VENC_IRQ_STATUS_PAUSE) {
		...
		MODULE_MFV_PR_ERR("irq_status 0x%x", uValue);
		VDO_HW_WRITE(KVA_VENC_SW_PAUSE, 0);
		VDO_HW_WRITE(KVA_VENC_SW_HRST_N, 0);
		uValue = VDO_HW_READ(KVA_VENC_SW_HRST_N);
		MODULE_MFV_PR_ERR("3 HRST = %d, isr = 0x%x", uValue, gu4HwVencIrqStatus);
	} else {		/* step 4 */
		VDO_HW_WRITE(KVA_VENC_SW_HRST_N, 0);
		uValue = gu4HwVencIrqStatus;
		VDO_HW_WRITE(KVA_VENC_SW_PAUSE, 0);
		MODULE_MFV_PR_DEBUG("4 HRST = %d, isr = 0x%x", uValue, gu4HwVencIrqStatus);
	}
...
static int vcodec_flush(struct file *file, fl_owner_t id)
{
	...
	MODULE_MFV_PR_DEBUG("vcodec_flush pid = %d, MT8167Driver_Open_Count %d\n",
...
static int vcodec_release(struct inode *inode, struct file *file)
{
	...
	/* dump_stack(); */
	MODULE_MFV_PR_DEBUG("[VCODEC_DEBUG] vcodec_release, curr_tid =%d\n", current->pid);
	mutex_lock(&DriverOpenCountLock);
	MODULE_MFV_PR_DEBUG("vcodec_flush pid = %d, MT8167Driver_Open_Count %d\n", current->pid,
		MT8167Driver_Open_Count);
	MT8167Driver_Open_Count--;
	...
	if (MT8167Driver_Open_Count == 0) {
		...
		if (gu4L2CCounter != 0) {
			...
			if (config_L2(1))
				MODULE_MFV_PR_ERR("restore L2 settings failed\n");
		}
		gu4L2CCounter = 0;
		mutex_unlock(&L2CLock);
#endif
...
static int vcodec_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	if (((length > VENC_REGION) || (pfn < VENC_BASE) || (pfn > VENC_BASE + VENC_REGION)) &&
		...
		if (u4I == VCODEC_MULTIPLE_INSTANCE_NUM_x_10) {
			MODULE_MFV_PR_ERR("[ERROR] mmap region error: Length(0x%lx), pfn(0x%lx)\n",
	...
	MODULE_MFV_PR_DEBUG("[mmap] vma->start 0x%lx, vma->end 0x%lx, vma->pgoff 0x%lx\n",
...
{
	mutex_lock(&PWRLock);
	MODULE_MFV_PR_DEBUG("vcodec_early_suspend, tid = %d, PWR_USER = %d\n", current->pid, gu4PWRCounter);
	mutex_unlock(&PWRLock);
	/*if (gu4PWRCounter != 0)
	* {
	* MODULE_MFV_PR_ERR("[MT6589_VCodec_early_suspend] Someone Use HW, Disable Power!\n");
	...
	* }
	*/
	MODULE_MFV_PR_DEBUG("vcodec_early_suspend - tid = %d\n", current->pid);
}

static void vcodec_late_resume(struct early_suspend *h)
{
	mutex_lock(&PWRLock);
	MODULE_MFV_PR_DEBUG("vcodec_late_resume, tid = %d, PWR_USER = %d\n", current->pid, gu4PWRCounter);
	mutex_unlock(&PWRLock);
	/*if (gu4PWRCounter != 0)
	* {
	* MODULE_MFV_PR_ERR("[vcodec_late_resume] Someone Use HW, Enable Power!\n");
	...
	* }
	*/
	MODULE_MFV_PR_DEBUG("vcodec_late_resume - tid = %d\n", current->pid);
}
...
static int vcodec_probe(struct platform_device *pdev)
{
	...
	*/
	...
	if (ret)
		MODULE_MFV_PR_ERR("[VCODEC_DEBUG][ERROR] Can't Get Major number for VCodec Device\n");
	...
	if (ret)
		MODULE_MFV_PR_ERR("[VCODEC_DEBUG][ERROR] Can't add Vcodec Device\n");

	vcodec_class = class_create(THIS_MODULE, VCODEC_DEVNAME);
	if (IS_ERR(vcodec_class)) {
		ret = PTR_ERR(vcodec_class);
		MODULE_MFV_PR_ERR("Unable to create class, err = %d", ret);
		return ret;
	}
	...
	*/
	if (request_irq
	    (VDEC_IRQ_ID, (irq_handler_t) video_intr_dlr, IRQF_TRIGGER_LOW, VCODEC_DEVNAME,
	     NULL) < 0) {
		MODULE_MFV_PR_DEBUG("[VCODEC_DEBUG][ERROR] error to request dec irq\n");
	} else {
		MODULE_MFV_PR_DEBUG("[VCODEC_DEBUG] success to request dec irq: %d\n", VDEC_IRQ_ID);
	}
	...
	*/
	if (request_irq
	    (VENC_IRQ_ID, (irq_handler_t) video_intr_dlr2, IRQF_TRIGGER_LOW, VCODEC_DEVNAME,
	     NULL) < 0) {
		MODULE_MFV_PR_DEBUG("[VCODEC_DEBUG][ERROR] error to request enc irq: %d\n", VENC_IRQ_ID);
	} else {
		MODULE_MFV_PR_DEBUG("[VCODEC_DEBUG] success to request enc irq: %d\n", VENC_IRQ_ID);
	}
	...
	if (request_irq
	    (VENC_LT_IRQ_ID, (irq_handler_t) video_intr_dlr2, IRQF_TRIGGER_LOW, VCODEC_DEVNAME,
	     NULL) < 0) {
		MODULE_MFV_PR_DEBUG("[VCODEC_DEBUG][ERROR] error to request enc_LT irq\n");
	} else {
		MODULE_MFV_PR_DEBUG("[VCODEC_DEBUG] success to request enc_LT irq: %d\n", VENC_LT_IRQ_ID);
	}
	#endif
	...
	MODULE_MFV_PR_DEBUG("[VCODEC_DEBUG] vcodec_probe Done\n");
...
static int venc_enableIRQ(VAL_HW_LOCK_T *prHWLock)
{
	...
	MODULE_MFV_PR_DEBUG("venc_enableIRQ+\n");
	/*if (VAL_DRIVER_TYPE_H264_ENC == prHWLock->eDriverType
	 *  || VAL_DRIVER_TYPE_HEVC_ENC == prHWLock->eDriverType)
	 *	 u4IrqId = VENC_IRQ_ID;
	 *else if (VAL_DRIVER_TYPE_VP8_ENC == prHWLock->eDriverType)
	...
	if (prHWLock->bSecureInst == VAL_FALSE) {
		MODULE_MFV_PR_DEBUG("[VCODEC_LOCKHW]  ENC Request IR by type 0x%x\n", prHWLock->eDriverType);
		if (request_irq(
			u4IrqId,
			(irq_handler_t)video_intr_dlr2,
			IRQF_TRIGGER_LOW,
			VCODEC_DEVNAME, NULL) < 0)	{
			MODULE_MFV_PR_ERR("[VCODEC_LOCKHW] ENC [MFV_DEBUG][ERROR] error to request enc irq\n");
		 } else{
			 MODULE_MFV_PR_DEBUG("[VCODEC_LOCKHW] ENC [MFV_DEBUG] success to request enc irq\n");
		 }
	 }
#else
	enable_irq(u4IrqId);
...
static int __init vcodec_driver_init(void)
{
	...
	}
	{
		...
		MODULE_MFV_PR_DEBUG("[DeviceTree] VDEC_IRQ_ID(%d), VENC_IRQ_ID(%d)", VDEC_IRQ_ID, VENC_IRQ_ID);
	}
...
/* KVA_VENC_IRQ_ACK_ADDR  = (VAL_ULONG_T)ioremap(VENC_IRQ_ACK_addr, 4); */
	...
	if (eValHWLockRet != VAL_RESULT_NO_ERROR)
		MODULE_MFV_PR_ERR("[MFV][ERROR] create dec hwlock event error\n");
	...
	if (eValHWLockRet != VAL_RESULT_NO_ERROR)
		MODULE_MFV_PR_ERR("[MFV][ERROR] create enc hwlock event error\n");
	...
	if (eValHWLockRet != VAL_RESULT_NO_ERROR)
		MODULE_MFV_PR_ERR("[MFV][ERROR] create dec isr event error\n");
	...
	if (eValHWLockRet != VAL_RESULT_NO_ERROR)
		MODULE_MFV_PR_ERR("[MFV][ERROR] create enc isr event error\n");

	MODULE_MFV_PR_DEBUG("[VCODEC_DEBUG] vcodec_driver_init Done\n");
...
static void __exit vcodec_driver_exit(void)
{
	...
	MODULE_MFV_PR_DEBUG("[VCODEC_DEBUG] vcodec_driver_exit\n");

	mutex_lock(&IsOpenedLock);
	if (bIsOpened == VAL_TRUE) {
		MODULE_MFV_PR_DEBUG("+vcodec_driver_exit remove device !!\n");
#ifdef CONFIG_OF
		...
		*/
		...
		bIsOpened = VAL_FALSE;
#endif
		MODULE_MFV_PR_DEBUG("+vcodec_driver_exit remove done !!\n");
	}
	mutex_unlock(&IsOpenedLock);
	...
	if (eValHWLockRet != VAL_RESULT_NO_ERROR)
		MODULE_MFV_PR_ERR("[MFV][ERROR] close dec hwlock event error\n");

	eValHWLockRet = eVideoCloseEvent(&EncHWLockEvent, sizeof(VAL_EVENT_T));
	if (eValHWLockRet != VAL_RESULT_NO_ERROR)
		MODULE_MFV_PR_ERR("[MFV][ERROR] close enc hwlock event error\n");

	/* MT6589_IsrEvent part */
	eValHWLockRet = eVideoCloseEvent(&DecIsrEvent, sizeof(VAL_EVENT_T));
	if (eValHWLockRet != VAL_RESULT_NO_ERROR)
		MODULE_MFV_PR_ERR("[MFV][ERROR] close dec isr event error\n");

	eValHWLockRet = eVideoCloseEvent(&EncIsrEvent, sizeof(VAL_EVENT_T));
	if (eValHWLockRet != VAL_RESULT_NO_ERROR)
		MODULE_MFV_PR_ERR("[MFV][ERROR] close enc isr event error\n");
#ifdef CONFIG_HAS_EARLYSUSPEND
