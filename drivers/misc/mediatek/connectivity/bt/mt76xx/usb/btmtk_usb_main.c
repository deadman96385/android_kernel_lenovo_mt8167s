...
static int btmtk_skb_enq_fwlog(void *src, u32 len, struct sk_buff_head *queue)
{
	struct sk_buff *skb_tmp = NULL;
	ulong flags = 0;

	skb_tmp = alloc_skb(len, GFP_ATOMIC);
	if (skb_tmp == NULL) {
...
static int btmtk_usb_dispatch_event(u8 *buf, int len)
{
	...
	if (buf[0] == 0xFF && buf[1] + 2 == len && (buf[2] == 0x50 || buf[2] == 0x51)) {
		if (skb_queue_len(&g_data->fwlog_queue) < FWLOG_QUEUE_COUNT) {
			...
			if (btmtk_skb_enq_fwlog(buf, len, &g_data->fwlog_queue) == 0) {
...
static void btmtk_usb_hci_snoop_print_to_log(void)
{
	...
	BTUSB_INFO("%s HCI Command Dump", __func__);
	BTUSB_INFO("%s index(len)(timestamp:us) :HCI Command", __func__);
...
static int btmtk_usb_load_woble_block_setting(char *block_name, struct woble_setting_struct *save_content,
			int save_content_count, u8 *searchcontent)
{
	int ret = 0;
	int i = 0;
	...
	for (i = 0; i < WOBLE_SETTING_COUNT; i++) {
		...
		search_result = strstr(searchcontent, search);
		if (search_result) {
			memset(temp, 0, sizeof(temp));
			temp_len = 0;
			...
			do {
				...
				if (ret == 0) {
					...
					BTUSB_DBG("%s:kstrtol ret = %d", __func__, ret);
		...
		if (temp_len) {
			...
			BTUSB_DBG("%s:x  save_content[%d].length %d temp_len=%d",
					 ...
					 struct device *dev, u32 *code_len, struct btmtk_usb_data *data)
{
	...
	*code_len = fw_entry->size;
	...
	err = btmtk_usb_load_woble_block_setting("RADIOOFF",
			&data->woble_setting_radio_off, 1, image);
	if (err)
		goto LOAD_END;
...
static int btmtk_usb_handle_leaving_WoBLE_state(void)
{
	...
	BTUSB_INFO("%s: g_data->woble_setting_radio_on.length %d",
		__func__, g_data->woble_setting_radio_on.length);
	if (g_data->woble_setting_radio_on.length) {
		...
		if (g_data->woble_setting_radio_on.length) {
			BTUSB_INFO_RAW(g_data->woble_setting_radio_on.content,
				g_data->woble_setting_radio_on.length, "%s: send radio on %d:", __func__, i);
			...
			if (g_data->woble_setting_radio_on_status_event.length) {
			...
			if (g_data->woble_setting_radio_on_comp_event.length) {
				...
				ret = btmtk_usb_wait_until_event(
						g_data->woble_setting_radio_on_comp_event.content,
		...
		if (g_data->woble_setting_radio_off.length && is_support_unify_woble(g_data)) {
			/* start to send radio off cmd from woble setting file */
				BTUSB_INFO_RAW(g_data->woble_setting_radio_off.content,
					g_data->woble_setting_radio_off.length, "send radio off %d:", i);
				...
				if (g_data->woble_setting_radio_off_status_event.length) {
	...
	if (urb->actual_length > 4) {
		...
		if (buf[0] == 0x6f && buf[1] == 0xfc && len + 4 == urb->actual_length) {
			...
			if (buf[urb->actual_length - 6] == ' ' &&
				...
				/*btmtk_usb_toggle_rst_pin()is called in btmtk_fifo_in*/
				block_picus_warn = 0;
			}
...
static int btmtk_usb_send_get_vendor_cap(void)
{
	...
	u8 get_vendor_cap_event[] = { 0x0e, 0x12, 0x01, 0x53, 0xFD, 0x00, 0x64, 0x01, 0xb0, 0x4f, 0x32,
		0x01 };
...
static ssize_t btmtk_usb_fops_read(struct file *file, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if ((g_data->i_buf[4] == 0x61 && g_data->i_buf[5] == 0xfc) ||
			(g_data->i_buf[4] == 0x11 && g_data->i_buf[5] == 0xfd)) {
		if (skb_queue_len(&g_data->fwlog_queue) < FWLOG_ASSERT_QUEUE_COUNT) {
			/* sent RSSI data to queue, picus tool will log it */
			skb_fwlog = alloc_skb(copyLen, GFP_ATOMIC);
			if (skb_fwlog == NULL) {
				BTUSB_ERR("%s: alloc_skb return 0, error", __func__);
				goto OUT;
			}
			memcpy(&skb_fwlog->data[0], &g_data->i_buf[0], copyLen);
			skb_fwlog->len = copyLen;
			spin_lock_irqsave(&g_data->fwlog_lock, flags);
			skb_queue_tail(&g_data->fwlog_queue, skb_fwlog);
			spin_unlock_irqrestore(&g_data->fwlog_lock, flags);
			wake_up_interruptible(&fw_log_inq);
		} else {
			BTUSB_WARN("%s: FW queue size is full", __func__);
		}
	}
