...
static UINT32 consys_store_reset_control(struct platform_device *pdev);
static MTK_WCN_BOOL consys_need_gps(VOID);
...
WMT_CONSYS_IC_OPS consys_ic_ops = {
	...
	.consys_ic_store_reset_control = consys_store_reset_control,
	.consys_ic_need_gps = consys_need_gps,
};
/*******************************************************************************
...
static INT32 consys_pmic_get_from_dts(struct platform_device *pdev)
{
	...
	reg_VCN35 = regulator_get(&pdev->dev, "vcn35");
	if (!reg_VCN35)
		WMT_PLAT_ERR_FUNC("Regulator_get VCN_3V5 fail\n");
#endif
#endif
#endif
	return 0;
...
static UINT32 consys_emi_set_remapping_reg(VOID)
{
	...
	addrPhy = (gConEmiPhyBase >> 20) & 0x0FFF;
...
static INT32 consys_read_reg_from_dts(VOID)
{
	...
	if (node) {
		...
		WMT_PLAT_ERR_FUNC("Get mcu register base(0x%zx)\n", conn_reg.mcu_base);
		i++;
#if CONFIG_RESET_CONTROL
#else
		conn_reg.ap_rgu_base = (SIZE_T) of_iomap(node, i);
		WMT_PLAT_ERR_FUNC("Get ap_rgu register base(0x%zx)\n", conn_reg.ap_rgu_base);
		i++;
#endif
		conn_reg.topckgen_base = (SIZE_T) of_iomap(node, i);
		WMT_PLAT_ERR_FUNC("Get topckgen register base(0x%zx)\n", conn_reg.topckgen_base);
		i++;
#if CONSYS_PWR_ON_OFF_API_AVAILABLE
#else
		conn_reg.spm_base = (SIZE_T) of_iomap(node, i);
		WMT_PLAT_ERR_FUNC("Get spm register base(0x%zx)\n", conn_reg.spm_base);
#endif
	} else {
		WMT_PLAT_ERR_FUNC("[%s] can't find CONSYS compatible node\n", __func__);
		return iRet;
...
static MTK_WCN_BOOL consys_need_gps(VOID)
{
	WMT_PLAT_WARN_FUNC("GPS/FM does not support on this platform\n");
