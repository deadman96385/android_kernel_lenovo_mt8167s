...
static const unsigned int g_default_freq[] = {
	ARMCA7PLL_DEF_FREQ, MAINPLL_DEF_FREQ, MEMPLL_DEF_FREQ,
	MMPLL_DEF_FREQ, TVDPLL_DEF_FREQ, LVDSPLL_DEF_FREQ
};
...
static const unsigned int g_ssc_setting_size[] = {
	...
	ARRAY_SIZE(ssc_lvdspll_setting),
};
...
static const unsigned long g_reg_pll_con1[] = {
	REG_ARMCA7PLL_CON1, REG_MAINPLL_CON1, REG_MEMPLL_CON1,
	REG_MMPLL_CON1, REG_TVDPLL_CON1, REG_LVDSPLL_CON1};
	...
	/* FH_MSG("5. write back to ncpo"); */
	{
		...
		if (pll_id == FH_MEM_PLLID) {
			...
			fh_write32(reg_pll_con1,
				((fh_read32(g_reg_mon[pll_id])&MASK21b) << 10)   /* /< left shift 10bit to [30:20] */
...
static void mt_fh_hal_popod_save(void)
{
	...
	if (g_fh_pll[pll_id].fh_status == FH_FH_ENABLE_SSC) {
		...
		fh_write32(g_reg_pll_con1[pll_id],
			(fh_read32(g_reg_dds[pll_id])&MASK21b)|(fh_read32(REG_MAINPLL_CON1)&0xFFE00000)|(BIT32));
			FH_MSG("REG_MAINPLL_CON1: 0x%08x", (fh_read32(g_reg_pll_con1[pll_id])&MASK21b));
...
static int fh_dumpregs_proc_read(struct seq_file *m, void *v)
{
	...
	seq_puts(m, "\r\nPLL_CON0 :\r\n");
	seq_printf(m, "ARMCA7:0x%08x MAIN:0x%08x MEM:0x%08x ",
			fh_read32(REG_ARMCA7PLL_CON0), fh_read32(REG_MAINPLL_CON0),	fh_read32(REG_MEMPLL_CON0));
	seq_printf(m, "MM:0x%08x TVD:0x%08x LVDS:0x%08x\r\n",
			fh_read32(REG_MMPLL_CON0), fh_read32(REG_TVDPLL_CON0), fh_read32(REG_LVDSPLL_CON0));

	seq_puts(m, "\r\nPLL_CON1 :\r\n");
	seq_printf(m, "ARMCA7:0x%08x MAIN:0x%08x MEM:0x%08x ",
			fh_read32(REG_ARMCA7PLL_CON1), fh_read32(REG_MAINPLL_CON1),	fh_read32(REG_MEMPLL_CON1));
	seq_printf(m, "MM:0x%08x TVD:0x%08x LVDS:0x%08x\r\n",
			fh_read32(REG_MMPLL_CON1), fh_read32(REG_TVDPLL_CON1), fh_read32(REG_LVDSPLL_CON1));
...
static int __reg_base_addr_init(void)
{
	...
	fhctl_node = of_find_compatible_node(NULL, NULL, "mediatek,mt8167-fhctl");
	if (!fhctl_node) {
	...
	apmixed_node = of_find_compatible_node(NULL, NULL, "mediatek,mt8167-apmixedsys");
	if (!fhctl_node) {
	...
	ddrphy_node = of_find_compatible_node(NULL, NULL, "mediatek,mt8167-ddrphy");
	if (!fhctl_node) {
		FH_MSG_DEBUG(" Error, Cannot find FHCTL device tree node");
		return -1;
	}
...
static int __fh_debug_proc_read(struct seq_file *m, void *v, fh_pll_t *pll)
{
	...
	seq_puts(m, "===============================================\r\n");
	seq_puts(m, "id=ARMCA7PLL=MAINPLL=MEMPLL=MSDCPLL=TVDPLL=LVDSPLL\r\n");
...
late_initcall(mt_freqhopping_drv_init);
