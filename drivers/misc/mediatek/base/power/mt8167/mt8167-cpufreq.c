 ...
 * Author: Andrew-sh.Chneg <andrew-sh.cheng@mediatek.com>
 *
...
static int mt8167_cpufreq_probe(struct platform_device *pdev)
{
	struct mtk_cpu_dvfs_info *info;
	...
	ret = cpufreq_register_driver(&mt8167_cpufreq_driver);
	if (ret) {
...
static struct platform_driver mt8167_cpufreq_platdrv = {
	.driver = {
		.name	= "mt8167-cpufreq",
	},
	.probe		= mt8167_cpufreq_probe,
};

static int mt8167_cpufreq_driver_init(void)
{
	...
	err = platform_driver_register(&mt8167_cpufreq_platdrv);
	if (err)
		return err;
	 ...
	 * probing.
	 */
	pdev = platform_device_register_simple("mt8167-cpufreq", -1, NULL, 0);
	if (IS_ERR(pdev)) {
...
device_initcall(mt8167_cpufreq_driver_init);
