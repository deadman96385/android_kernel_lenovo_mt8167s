...
/* UDI pin mux ADDR */
/* 1. Write 0x10005380 bit[14:12]=0x6 to switch UDI pin, TMS */
/* 2. Write 0x10005380 bit[11:9]=0x6 to switch UDI pin, NTRST */
/* 3. Write 0x10005390 bit[8:6]=0x6 to switch UDI pin, TDO */
/* 4. Write 0x10005390 bit[5:3]=0x6 to switch UDI pin, TDI */
/* 5. Write 0x10005390 bit[2:0]=0x6 to switch UDI pin, TCK */
...
#define UDIPIN_UDI_JTAG1			(UDIPIN_BASE+0x380)
#define UDIPIN_UDI_JTAG1_VALUE		(0x00006C01)
#define UDIPIN_UDI_JTAG1_DEFAULT	(0x00001001)
#define UDIPIN_UDI_JTAG2			(UDIPIN_BASE+0x390)
#define UDIPIN_UDI_JTAG2_VALUE		(0x000001B6)
#define UDIPIN_UDI_JTAG2_DEFAULT	(0x00000049)
...
static int udi_pinmux_proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "UDI pinmux reg[0x%p] = 0x%x.\n", UDIPIN_UDI_JTAG2, _udi_read(UDIPIN_UDI_JTAG2));
	return 0;
}
...
static ssize_t udi_pinmux_proc_write(struct file *file, const char __user *buffer, size_t count, loff_t *pos)
{
	...
	if (!kstrtoint(buf, 10, &pin_switch)) {
		if (pin_switch == 1) {
			...
			_udi_write(UDIPIN_UDI_JTAG2, UDIPIN_UDI_JTAG2_VALUE);
		} else {
			/* default */
			_udi_write(UDIPIN_UDI_JTAG1, UDIPIN_UDI_JTAG1_DEFAULT);
			_udi_write(UDIPIN_UDI_JTAG2, UDIPIN_UDI_JTAG2_DEFAULT);
		}
	} else
...
static int __init udi_init(void)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,mt8167-pctl-a-syscfg");
	if (!node) {
