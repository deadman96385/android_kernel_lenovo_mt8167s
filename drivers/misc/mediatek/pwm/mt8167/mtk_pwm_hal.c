/*
 * mt8167 mtk_pwm_hal.c PWM Drvier
 *
...
void mt_pwm_power_on_hal(u32 pwm_no, bool pmic_pad, unsigned long *power_flag)
{
	...
	if (0 == (*power_flag)) {
		...
		if (clk_en_ret) {
			pr_err
			    ("[PWM][CCF]enable clk PWM_HCLK failed. ret:%d, clk_pwm_hclk_main:%p\n",
