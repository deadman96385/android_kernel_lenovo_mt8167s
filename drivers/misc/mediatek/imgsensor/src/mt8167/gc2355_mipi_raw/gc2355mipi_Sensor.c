...
static imgsensor_info_struct imgsensor_info = {
	...
	.checksum_value = 0x31fb1318,	/* checksum value for Camera Auto Test */
	...
	.cap1 = {
	/* capture for PIP 24fps relative information, capture1 mode must use same framelength, l
	inelength with Capture mode for shutter calculate */
		 .pclk = 42000000,
		 .linelength = 1120,
		 .framelength = 1246,
		 .startx = 0,
...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
	...
	{1600, 1200, 0, 0, 1600, 1200, 1600, 1200, 0000, 0000, 1600, 1200, 0, 0, 1280, 720}
};				/* slim video */
...
static void preview_setting(kal_uint16 currefps)
{
	...
	if (currefps != 0) {
		frame_length =
		    imgsensor_info.pre.pclk / imgsensor.current_fps * 10 /
	...
	hb = imgsensor.dummy_pixel + GC2355_DEFAULT_DUMMY_PIXEL_NUMS;
	vb = imgsensor.dummy_line + GC2355_DEFAULT_DUMMY_LINE_NUMS;
	LOG_INF("dummyline = %d(%d), dummypixels = %d(%d)\n", imgsensor.dummy_line, vb,
		imgsensor.dummy_pixel, hb);
	/* AEC&frame length// */
...
static void normal_video_setting(kal_uint16 currefps)
{
	...
	if (currefps != 0) {
		frame_length =
		    imgsensor_info.normal_video.pclk / imgsensor.current_fps * 10 /
