...
#define RCMM_INFO_TO_BITCNT(u4Info)		((u4Info & IRRX_CH_BITCNT_MASK)    >> IRRX_CH_BITCNT_BITSFT)
#define RCMM_INFO_TO_1STPULSE(u4Info)	((u4Info & IRRX_CH_1ST_PULSE_MASK) >> IRRX_CH_1ST_PULSE_BITSFT)
#define RCMM_INFO_TO_2NDPULSE(u4Info)	((u4Info & IRRX_CH_2ND_PULSE_MASK) >> IRRX_CH_2ND_PULSE_BITSFT)
#define RCMM_INFO_TO_3RDPULSE(u4Info)	((u4Info & IRRX_CH_3RD_PULSE_MASK) >> IRRX_CH_3RD_PULSE_BITSFT)
...
#define IR_RCMM_ERR(fmt, args...)    pr_err(MTK_IR_TAG"%s %d : "fmt, __func__, __LINE__, ##args)
...
static int mtk_ir_rcmm_timer_func(const char *data)
{
	...
	if (CHK_CNT_PULSE == IRRX_IRCHK_CNT) {
		_u4Routine_count++;
		if (data[0])
			IR_RCMM_ERR("CHK_CNT_PULSE = 0x%x, _u4Routine_count = %d\n",
...
static int mtk_ir_rcmm_init_hw(void)
{
	...
	IR_WRITE32(IRRX_EXP_IRM1, MTK_RCMM_EXP_POWE_KEY1);
	IR_WRITE32(IRRX_EXP_IRL1, MTK_RCMM_EXP_POWE_KEY2);
	...
	IR_RCMM_LOG("rcmm_threshold_reg:0x%08x, rcmm_threshold_reg0:0x%08x\n", IR_READ32(IRRX_RCMM_THD_REG),
		   IR_READ32(IRRX_RCMM_THD_REG0));

	IR_RCMM_LOG("mtk_ir_rcmm_init_hw----\n");
	return 0;
}
...
static int ir_rcmm_local_init(void)
{
	...
	IR_RCMM_FUN();
	err += mtk_ir_register_ctl_data_path(&mtk_ir_pdata_rcmm, hw, RC_TYPE_RCMM);
	IR_RCMM_LOG("ir_rcmm_local_init(%d)----\n", err);
	return err;
}
