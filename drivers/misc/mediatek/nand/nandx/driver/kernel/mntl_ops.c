			       ...
			       u32 block, u32 page, u32 offset, u32 size)
{
	...
	if (size < page_size) {
		...
		memcpy(oob_buffer, fdm_buf + start_sect * 8, sect_read * 8);
#else
		ops.row = page_addr;
		ops.col = col_addr;
		ops.len = size;
		...
		ops.oob = fdm_buf;
		ret = nandx_core_read(&ops, 1, mode);
		memcpy(oob_buffer, fdm_buf, sect_read * 8);
#endif
	} else {
		ops.row = page_addr;
		ops.col = 0;
			...
			struct list_node *start_node, int count)
{
	...
	ret = count;
	item = start_node;
	for (i = 0; i < info->max_keep_pages; i++) {
		work = get_list_work(item);
		ops = &work->ops;
		item = item->next;
		if (i == 0)
	...
	status = nandx_core_write(ops_table, info->max_keep_pages, mode);
				 ...
				 struct worklist_ctrl *list_ctrl, int count)
{
	...
	mode |= MODE_SLC;
	if (info->plane_num > 1 && (count % info->plane_num) == 0)
		mode |= MODE_MULTI;
