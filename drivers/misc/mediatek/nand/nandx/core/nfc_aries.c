...
static inline void nfc_dump_register(struct nfc_info *info)
{
	...
	pr_debug("NFI_PAGEFMT = 0x%x\n", nfi_readl(info, NFI_PAGEFMT));
	pr_debug("NFI_CON = 0x%x\n", nfi_readl(info, NFI_CON));
	pr_debug("NFI_ACCCON = 0x%x\n", nfi_readl(info, NFI_ACCCON));
	pr_debug("NFI_INTR_EN = 0x%x\n", nfi_readl(info, NFI_INTR_EN));
	pr_debug("NFI_INTR_STA = 0x%x\n", nfi_readl(info, NFI_INTR_STA));
	pr_debug("NFI_CMD = 0x%x\n", nfi_readl(info, NFI_CMD));
	pr_debug("NFI_ADDRNOB = 0x%x\n", nfi_readl(info, NFI_ADDRNOB));
	pr_debug("NFI_COLADDR = 0x%x\n", nfi_readl(info, NFI_COLADDR));
	pr_debug("NFI_ROWADDR = 0x%x\n", nfi_readl(info, NFI_ROWADDR));
	pr_debug("NFI_CNRNB = 0x%x\n", nfi_readl(info, NFI_CNRNB));
	pr_debug("NFI_PIO_DIRDY = 0x%x\n", nfi_readl(info, NFI_PIO_DIRDY));
	pr_debug("NFI_STA = 0x%x\n", nfi_readl(info, NFI_STA));
	pr_debug("NFI_ADDRCNTR = 0x%x\n", nfi_readl(info, NFI_ADDRCNTR));
	pr_debug("NFI_STRADDR = 0x%x\n", nfi_readl(info, NFI_STRADDR));
	pr_debug("NFI_BYTELEN = 0x%x\n", nfi_readl(info, NFI_BYTELEN));
	pr_debug("NFI_CSEL = 0x%x\n", nfi_readl(info, NFI_CSEL));
	pr_debug("NFI_FDML0 = 0x%x\n", nfi_readl(info, NFI_FDML(0)));
	pr_debug("NFI_FDMM0 = 0x%x\n", nfi_readl(info, NFI_FDMM(0)));
	pr_debug("NFI_DEBUG_CON1 = 0x%x\n", nfi_readl(info, NFI_DEBUG_CON1));
	pr_debug("NFI_MASTER_STA = 0x%x\n", nfi_readl(info, NFI_MASTER_STA));
	pr_debug("NFI_SECCUS_SIZE = 0x%x\n", nfi_readl(info, NFI_SECCUS_SIZE));
	pr_debug("NFI_RANDOM_CNFG = 0x%x\n", nfi_readl(info, NFI_RANDOM_CNFG));
	pr_debug("NFI_EMPTY_THRESH = 0x%x\n",
		  nfi_readl(info, NFI_EMPTY_THRESH));
	pr_debug("NFI_NAND_TYPE_CNFG = 0x%x\n",
		  nfi_readl(info, NFI_NAND_TYPE_CNFG));
	pr_debug("NFI_ACCCON1 = 0x%x\n", nfi_readl(info, NFI_ACCCON1));
	pr_debug("NFI_DELAY_CTRL = 0x%x\n", nfi_readl(info, NFI_DELAY_CTRL));
	pr_debug("NFI_TLC_RD_WHR2 = 0x%x\n", nfi_readl(info, NFI_TLC_RD_WHR2));

	pr_debug("ECC_ENCCON = 0x%x\n", ecc_readl(info, ECC_ENCCON));
	pr_debug("ECC_ENCCNFG = 0x%x\n", ecc_readl(info, ECC_ENCCNFG));
	pr_debug("ECC_ENCDIADDR = 0x%x\n", ecc_readl(info, ECC_ENCDIADDR));
	pr_debug("ECC_ENCIDLE = 0x%x\n", ecc_readl(info, ECC_ENCIDLE));
	pr_debug("ECC_ENCSTA = 0x%x\n", ecc_readl(info, ECC_ENCSTA));
	pr_debug("ECC_ENCIRQ_EN = 0x%x\n", ecc_readl(info, ECC_ENCIRQ_EN));
	pr_debug("ECC_ENCIRQ_STA = 0x%x\n", ecc_readl(info, ECC_ENCIRQ_STA));
	pr_debug("ECC_PIO_DIRDY = 0x%x\n", ecc_readl(info, ECC_PIO_DIRDY));
	pr_debug("ECC_DECCON = 0x%x\n", ecc_readl(info, ECC_DECCON));
	pr_debug("ECC_DECCNFG = 0x%x\n", ecc_readl(info, ECC_DECCNFG));
	pr_debug("ECC_DECDIADDR = 0x%x\n", ecc_readl(info, ECC_DECDIADDR));
	pr_debug("ECC_DECIDLE = 0x%x\n", ecc_readl(info, ECC_DECIDLE));
	pr_debug("ECC_DECENUM0 = 0x%x\n", ecc_readl(info, ECC_DECENUM(0)));
	pr_debug("ECC_DECDONE = 0x%x\n", ecc_readl(info, ECC_DECDONE));
	pr_debug("ECC_DECIRQ_EN = 0x%x\n", ecc_readl(info, ECC_DECIRQ_EN));
	pr_debug("ECC_DECIRQ_STA = 0x%x\n", ecc_readl(info, ECC_DECIRQ_STA));
	pr_debug("ECC_DECFSM = 0x%x\n", ecc_readl(info, ECC_DECFSM));
}
...
static int ecc_runtime_config(struct nfc_info *info)
{
	...
	for (i = 0; i < info->ecc_strength_num; i++) {
		if (info->ecc_strength[i] == info->format.ecc_strength)
			break;
	}
...
static int nfc_setup_ecc_clk(struct nfc_info *info, struct nfc_frequency *freq)
{
	...
	if (!info->ecc_clk_en) {
		pr_debug("%s: not support ecc clk\n",
			    __func__);
		return 0;
	}
	...
	if (info->res->ver == NANDX_MT8167 && !info->mode.dma_en) {
		pr_debug("%s: do not enable ecc clk\n",
			    __func__);
		return 0;
	}

	if (freq->ecc_clk_num < 0 || freq->sel_ecc_idx >= freq->ecc_clk_num) {
		pr_debug("%s: invalid clk, num %d, idx %d\n",
					...
					struct nfc_frequency *freq)
{
	...
	trlt = MAX(legacy->tREA, legacy->tRP);
					...
					struct nfc_frequency *freq)
{
	...
	u32 trdpre, trdpst, twrpre, twrpst;
	...
	if (freq->sel_2x_idx < 0) {
		...
		for (i = 0; i < freq->nfi_clk_num; i++) {
			/*
			 * Todo:
			 * chip info should provide the max frequency supported.
			 */
			if (freq->freq_2x[i] > 166000000 ||
			    freq->freq_2x[i] <= rate)
				continue;
			rate = freq->freq_2x[i];
			idx = i;
	...
	if (onfi->tWPRE > temp)
		tprecs = MAX(tprecs, onfi->tWPRE - temp);
	tprecs = div_up(tprecs * rate, 1000000);
	tprecs &= 0xf;
	tc2r = 0;
	tw2r = onfi->tWHR;
	...
	/* set NFI_ACCCON1 */
	trdpre = 0;
	trdpst = div_up(onfi->tCKWR * rate, 1000000) - 1;
	trdpst &= 0x3f;
	twrpre = 0;
	twrpst = div_up(onfi->tWPST * rate, 1000000) - 1;
	twrpst &= 0x1f;
			 ...
			 struct nand_timing *timing, void *arg)
{
	...
	case INTERFACE_LEGACY:
		ret = nfc_change_legacy_interface(info, timing->legacy, freq);
		break;
	case INTERFACE_ONFI:
		ret = nfc_change_onfi_interface(info, timing->ddr.onfi, freq);
		break;
	case INTERFACE_TOGGLE:
		ret = nfc_change_toggle_interface(info, timing->ddr.toggle,
						  freq);
		break;
	default:
		ret = -EINVAL;
