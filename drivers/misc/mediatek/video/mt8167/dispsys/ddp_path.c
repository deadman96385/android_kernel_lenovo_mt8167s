...
struct mout_t mout_map[DDP_MOUT_NUM] = {
	/* [0]OVL_MOUT */
	{DISP_MODULE_OVL0, {{DISP_MODULE_COLOR0, 1 << 0}, {DISP_MODULE_WDMA0, 1 << 1}, {-1, 0} },
	 0, 0},
	/* [1]DITHER0_MOUT */
	{DISP_MODULE_DITHER,
	 {{DISP_MODULE_RDMA0, 1 << 0}, {DISP_MODULE_UFOE, 1 << 1}, {DISP_MODULE_WDMA0, 1 << 2}, {-1, 0} },
	 0, 0},
	/* [2]UFOE_MOUT */
	{DISP_MODULE_UFOE,
	 {{DISP_MODULE_DSI0, 1 << 0}, {DISP_MODULE_DPI0, 1 << 1},
	 {DISP_MODULE_DPI1, 1 << 2}, {DISP_MODULE_WDMA0, 1 << 3}, {-1, 0} },
	 0, 0},
};

struct sel_t sel_out_map[DDP_SEL_OUT_NUM] = {
	/* [0]RDMA0_SOUT */
	{DISP_MODULE_RDMA0,
	 {DISP_MODULE_UFOE, DISP_MODULE_COLOR0, DISP_MODULE_DSI0, DISP_MODULE_DPI0, DISP_MODULE_DPI1},
	 0, 0},
	/* [1]RDMA1_SOUT */
	{DISP_MODULE_RDMA1, {DISP_MODULE_DSI0, DISP_MODULE_DPI0, DISP_MODULE_DPI1, -1},
	0, 0},
};

/* 1st para is sout's output, 2nd para is sout's input */
struct sel_t sel_in_map[DDP_SEL_IN_NUM] = {
	/* [0]COLOR_SEL */
	{DISP_MODULE_COLOR0, {DISP_MODULE_RDMA0, DISP_MODULE_OVL0, -1},
	 0, 0},
	/* [1]UFOE_SEL */
	{DISP_MODULE_UFOE, {DISP_MODULE_RDMA0, DISP_MODULE_DITHER, -1},
	0, 0},
	/* [2]DSI0_SEL */
	{DISP_MODULE_DSI0, {DISP_MODULE_UFOE, DISP_MODULE_RDMA0, DISP_MODULE_RDMA1, -1},
	 0, 0},
	/* [3]DPI0_SEL */
	{DISP_MODULE_DPI0, {DISP_MODULE_UFOE, DISP_MODULE_RDMA0, DISP_MODULE_RDMA1, -1},
	 0, 0},
	/* [4]DPI1_SEL */
	{DISP_MODULE_DPI1, {DISP_MODULE_UFOE, DISP_MODULE_RDMA0, DISP_MODULE_RDMA1, -1},
	 0, 0},
	/* [5]WDMA_SEL */
	{DISP_MODULE_WDMA0, {DISP_MODULE_OVL0, DISP_MODULE_DITHER, DISP_MODULE_UFOE, -1},
	 0, 0},
};
...
int ddp_path_init(void)
{
	...
	/* mout */
	mout_map[index++].reg = (volatile unsigned long *)DISP_REG_CONFIG_DISP_OVL0_MOUT_EN;
#if defined(MTK_FB_OVL1_SUPPORT)
	mout_map[index++].reg = (volatile unsigned long *)DISP_REG_CONFIG_DISP_OVL1_MOUT_EN;
#endif
	mout_map[index++].reg = (volatile unsigned long *)DISP_REG_CONFIG_DISP_DITHER_MOUT_EN;
	mout_map[index++].reg = (volatile unsigned long *)DISP_REG_CONFIG_DISP_UFOE_MOUT_EN;
	/* sel_out */
	...
	sel_in_map[4].reg = (volatile unsigned long *)DISP_REG_CONFIG_DPI1_SEL_IN;	/* DPI1_SEL */
...
char *ddp_get_mutex_sof_name(enum MUTEX_SOF mode)
{
	switch (mode) {
	case SOF_SINGLE:
		return "single";
...
static enum MUTEX_SOF ddp_get_mutex_sof(enum DISP_MODULE_ENUM dest_module, enum DDP_MODE ddp_mode)
{
	enum MUTEX_SOF mode = SOF_SINGLE;
	...
	case DISP_MODULE_DPI0:
		{
			mode = SOF_DPI0;	/* SOF_DPI0;   has one DSI, so the DPI should use 1 for mutex_sof */
			break;
		}
...
static int ddp_mutex_set_l(int mutex_id, int *module_list, enum DDP_MODE ddp_mode, void *handle)
{
	...
	enum MUTEX_SOF mode = ddp_get_mutex_sof(module_list[module_num - 1], ddp_mode);
...
static void ddp_check_mutex_l(int mutex_id, int *module_list, enum DDP_MODE ddp_mode)
{
	...
	uint32_t real_sof = 0;
	enum MUTEX_SOF expect_sof = SOF_SINGLE;
	int module_num = ddp_get_module_num_l(module_list);
	...
	if ((uint32_t) expect_sof != real_sof) {
		DDPDUMP("error:mutex %d sof error: expect %s, real %s\n", mutex_id,
			...
			ddp_get_mutex_sof_name((enum MUTEX_SOF) real_sof));
	}
}

static int ddp_mutex_enable_l(int mutex_idx, void *handle)
{
	DDPDBG("mutex enable mutex[%d]\n", mutex_idx);
...
int ddp_mutex_disenable(int mutex_id, enum DDP_SCENARIO_ENUM scenario, void *handle)
{
