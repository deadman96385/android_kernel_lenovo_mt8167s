...
static void _MFC_Newline(struct MFC_CONTEXT *ctxt)
{
	...
	while (ctxt->cursor_col < ctxt->cols) {
		_MFC_DrawChar(ctxt, ctxt->cursor_col * MFC_FONT_WIDTH, ctxt->cursor_row * MFC_FONT_HEIGHT, ' ');
...
static void _MFC_Putc(struct MFC_CONTEXT *ctxt, const char c)
{
	...
	default: /* draw the char */
		_MFC_DrawChar(ctxt, ctxt->cursor_col * MFC_FONT_WIDTH, ctxt->cursor_row * MFC_FONT_HEIGHT, c);
		++ctxt->cursor_col;
		CHECK_NEWLINE();
	}
}
