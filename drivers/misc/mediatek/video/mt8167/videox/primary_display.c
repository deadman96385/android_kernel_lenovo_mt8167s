...
enum DISP_PRIMARY_PATH_MODE primary_display_mode = DECOUPLE_MODE;
#else
...
/* enum DDP_SCENARIO_ENUM ddp_scenario = DDP_SCENARIO_SUB_RDMA1_DISP; */
...
unsigned int gEnterSodiAfterEOF;

unsigned int WDMA0_FRAME_START_FLAG;
...
void reset_buffer(struct display_primary_path_context *ctx, struct disp_internal_buffer_info *buf)
{
	if (ctx && buf)
...
struct disp_internal_buffer_info *dequeue_buffer(struct display_primary_path_context *ctx,
					  struct list_head *head)
{
	struct disp_internal_buffer_info *temp = NULL;

	if (ctx && head) {
		if (!list_empty(head)) {
			temp = list_entry(head->prev, struct disp_internal_buffer_info, list);
...
struct disp_internal_buffer_info *find_buffer_by_mva(struct display_primary_path_context *ctx,
					      struct list_head *head, uint32_t mva)
{
	struct disp_internal_buffer_info *buf = NULL;
	struct disp_internal_buffer_info *temp = NULL;
...
static inline int _is_mirror_mode(enum DISP_MODE mode)
{
...
static inline int _is_decouple_mode(enum DISP_MODE mode)
{
...
static void primary_suspend_release_present_fence(void)
{
	...
	if (fence_increment > 0) {
		...
		mmprofile_log_ex(ddp_mmp_get_events()->present_fence_release,
			MMPROFILE_FLAG_PULSE, gPresentFenceIndex, fence_increment);
	}
	_primary_path_unlock(__func__);
...
int _disp_primary_path_set_vfp(int enter)
{
	...
	if (primary_display_is_video_mode()) {
		...
		struct cmdqRecStruct *cmdq_handle_vfp = NULL;
		...
		mmprofile_log_ex(ddp_mmp_get_events()->dal_clean, MMPROFILE_FLAG_PULSE, 0, enter);
...
int primary_display_save_power_for_idle(int enter, unsigned int need_primary_lock)
{
	...
	if (disp_low_power_disable_ddp_clock == 1) {/* open */
		if (primary_display_is_video_mode() == 0 /* && gEnableLowPowerFeature==1 */) {
			...
			if (enter == 1)	{ /* only for command mode */
				if (isIdlePowerOff == 0) {
...
static int _disp_primary_path_idle_detect_thread(void *data)
{
	...
	while (1) {
		...
		if (pgc->session_mode != DISP_SESSION_DIRECT_LINK_MODE) {
			DISPCHECK("[ddp_idle]primary display path is decouple mode. -- skip ddp_idle\n");
			_primary_path_unlock(__func__);
			continue;
		}
				...
				enum DISP_FRM_SEQ_STATE state)
{
	...
	if (state == FRM_CONFIG) {
		...
		mmprofile_log_ex(ddp_mmp_get_events()->primary_seq_config, MMPROFILE_FLAG_PULSE, addr, seq);

	} else if (state == FRM_TRIGGER) {
		frm_update_sequence[frm_update_cnt].state = FRM_TRIGGER;
		mmprofile_log_ex(ddp_mmp_get_events()->primary_seq_trigger, MMPROFILE_FLAG_PULSE,
		...
		for (i = 0; i < FRM_UPDATE_SEQ_CACHE_NUM; i++) {
			if ((abs(addr - frm_update_sequence[i].mva) <= frm_update_sequence[i].max_offset) &&
				...
				mmprofile_log_ex(ddp_mmp_get_events()->primary_seq_rdma_irq, MMPROFILE_FLAG_PULSE,
		...
		for (i = 0; i < FRM_UPDATE_SEQ_CACHE_NUM; i++) {
			if (frm_update_sequence[i].state == FRM_START) {
				...
				mmprofile_log_ex(ddp_mmp_get_events()->primary_seq_release, MMPROFILE_FLAG_PULSE,
 ...
 */
static unsigned int get_frm_seq_by_addr(unsigned int addr, enum DISP_FRM_SEQ_STATE state)
{
	int i = 0;
 ...
 * Defined but not used, avoid build warning. (2015.1.30 Rynn Wu)
 */
static unsigned int release_started_frm_seq(unsigned int addr, enum DISP_FRM_SEQ_STATE FRM_END)
{
	int i = 0;

	for (i = 0; i < FRM_UPDATE_SEQ_CACHE_NUM; i++) {
		if (frm_update_sequence[i].state == FRM_END)
			return frm_update_sequence[i].seq;
	}
	return 0;
}
...
{
	mmprofile_log_ex(ddp_mmp_get_events()->idlemgr_enter_idle, MMPROFILE_FLAG_PULSE, 0, 0);

	if (!primary_display_is_video_mode()) {
		spm_enable_sodi(1);
		spm_sodi_mempll_pwr_mode(0);
		mmprofile_log_ex(ddp_mmp_get_events()->sodi_enable, MMPROFILE_FLAG_PULSE, 0, 0);
	}
}

void primary_display_idlemgr_leave_idle(void)
{
	mmprofile_log_ex(ddp_mmp_get_events()->idlemgr_leave_idle, MMPROFILE_FLAG_PULSE, 0, 0);

	if (!primary_display_is_video_mode()) {
		spm_enable_sodi(0);
		mmprofile_log_ex(ddp_mmp_get_events()->sodi_disable, MMPROFILE_FLAG_PULSE, 0, 0);
	}
}
...
static void _dump_internal_buffer_into(struct display_primary_path_context *ctx, char *dump_reason)
{
	int n = 0;
	int len = 0;
	struct disp_internal_buffer_info *buf = NULL;
...
{
	struct disp_internal_buffer_info *reading_buf = NULL;
	struct disp_internal_buffer_info *temp = NULL;
	struct disp_internal_buffer_info *n = NULL;
...
{
	int ret = 0;
	struct primary_disp_input_config input;
	struct disp_internal_buffer_info *writing_buf = NULL;
	struct disp_internal_buffer_info *reading_buf = NULL;
	...
	while (1) {
		...
		memset((void *)&input, 0, sizeof(struct primary_disp_input_config));
		...
		input.isDirty = 1;
		mmprofile_log_ex(ddp_mmp_get_events()->interface_trigger, MMPROFILE_FLAG_PULSE,
...
static enum DISP_MODULE_ENUM _get_dst_module_by_lcm(struct disp_lcm_handle *plcm)
{
	...
	if (plcm->params->type == LCM_TYPE_DSI) {
		...
		if (plcm->lcm_if_id == LCM_INTERFACE_DPI0)
			return DISP_MODULE_DPI0;
		else if (plcm->lcm_if_id == LCM_INTERFACE_DPI1)
...
int _should_config_ovl_to_memory(struct display_primary_path_context *ctx)
{
	if (ctx == NULL) {
...
static void _cmdq_build_trigger_loop(void)
{
	...
	DISPPRINT("primary display BUILD cmdq trigger loop finished\n");
...
{
	mmprofile_log_ex(ddp_mmp_get_events()->cg_mode, MMPROFILE_FLAG_PULSE, 0, 0);
}

void disp_spm_enter_power_down_mode(void)
{
	mmprofile_log_ex(ddp_mmp_get_events()->power_down_mode, MMPROFILE_FLAG_PULSE, 0, 0);
}
...
static int primary_display_is_secure_path(enum DISP_SESSION_TYPE session_type)
{
	int i;
...
static int _build_path_rdma_to_dsi(void)
{
	...
	if (pgc->dpmgr_handle) {
		DISPCHECK("dpmgr create interface path SUCCESS\n");
	} else {
		DISPCHECK("dpmgr create path FAIL\n");
		return -1;
	}
...
static int _build_path_ovl_to_wdma(void)
{
	...
	if (pgc->ovl2mem_path_handle) {
		DISPCHECK("dpmgr create ovl memout path SUCCESS\n");
	} else {
		DISPCHECK("dpmgr create path FAIL\n");
		return -1;
	}
	...
	DISPCHECK("build rdma to dsi path finished\n");
	return ret;
}
...
int _trigger_ovl_to_memory(disp_path_handle disp_handle, struct cmdqRecStruct *cmdq_handle,
...
int _trigger_ovl_to_memory_mirror(disp_path_handle disp_handle, struct cmdqRecStruct *cmdq_handle,
...
struct task_struct *decouple_fence_release_task;
...
int _esd_check_config_handle_vdo(void)
{
	...
	/* write instruction(read from lcm) */
	dpmgr_path_build_cmdq(pgc->dpmgr_handle, pgc->cmdq_handle_config_esd, CMDQ_ESD_CHECK_READ);
...
int primary_display_esd_check(void)
{
		...
		if (primary_display_is_video_mode()) {
			...
			if (primary_display_cmdq_enabled()) {
				...
				dpmgr_path_build_cmdq(pgc->dpmgr_handle,
					pgc->cmdq_handle_config_esd, CMDQ_STOP_VDO_MODE);
				...
				dpmgr_path_build_cmdq(pgc->dpmgr_handle,
					pgc->cmdq_handle_config_esd, CMDQ_START_VDO_MODE);
			...
			mmprofile_log_ex(ddp_mmp_get_events()->esd_extte, MMPROFILE_FLAG_PULSE, 0, 1);
	...
	mmprofile_log_ex(ddp_mmp_get_events()->esd_rdlcm, MMPROFILE_FLAG_START, 0, primary_display_cmdq_enabled());
	if (primary_display_cmdq_enabled()) {
		_primary_path_lock(__func__);
		mmprofile_log_ex(ddp_mmp_get_events()->esd_rdlcm, MMPROFILE_FLAG_PULSE, 0, 1);
		/* 0.create esd check cmdq */
		cmdqRecCreate(CMDQ_SCENARIO_DISP_ESD_CHECK, &(pgc->cmdq_handle_config_esd));
		dpmgr_path_build_cmdq(pgc->dpmgr_handle, pgc->cmdq_handle_config_esd, CMDQ_ESD_ALLC_SLOT);
		mmprofile_log_ex(ddp_mmp_get_events()->esd_rdlcm, MMPROFILE_FLAG_PULSE, 0, 2);
...
int primary_display_esd_recovery(void)
{
	...
	mmprofile_log_ex(ddp_mmp_get_events()->esd_recovery_t, MMPROFILE_FLAG_PULSE, 0, 12);
...
void primary_display_esd_check_enable(int enable)
{
	...
	if (_is_enable_esd_check() != true) {
		DISPCHECK("[ESD]Checking if ESD enable but ESD check mechanism is not enabled.\n");
		return;
	}
...
unsigned int display_path_idle_cnt;
#if defined(MTK_FB_SODI_SUPPORT)
static void _RDMA0_INTERNAL_IRQ_Handler(enum DISP_MODULE_ENUM module, unsigned int param)
{
...
static void _WDMA0_INTERNAL_IRQ_Handler(enum DISP_MODULE_ENUM module, unsigned int param)
{
	if (param & 0x1) {
		/* WDMA Done */
...
static void _MUTEX_INTERNAL_IRQ_Handler(enum DISP_MODULE_ENUM module, unsigned int param)
{
...
{
#if 0 /*defined(MTK_FB_SODI_DEFEATURE)*/
...
static void _wdma_fence_release_callback(uint32_t userdata)
{
	...
	mmprofile_log_ex(ddp_mmp_get_events()->primary_wdma_fence_release, MMPROFILE_FLAG_PULSE, layer,
		       fence_idx);
}
...
static int _Interface_fence_release_callback(uint32_t userdata)
{
	...
	if (userdata > 0) {
		...
		mmprofile_log_ex(ddp_mmp_get_events()->primary_wdma_fence_release, MMPROFILE_FLAG_PULSE,
			       layer, userdata);
	}
...
static int _ovl_ext_fence_release_callback(uint32_t userdata)
{
	...
	for (i = 0; i < PRIMARY_DISPLAY_SESSION_LAYER_COUNT; i++) {
		...
		mmprofile_log_ex(ddp_mmp_get_events()->primary_ovl_fence_release, MMPROFILE_FLAG_PULSE, i, fence_idx);
	}
#endif
...
static int _present_fence_release_worker_thread(void *data)
{
	...
	while (1) {
		...
		/* release present fence in vsync callback */
		{
			...
			if (pgc->session_id == 0) {
				mmprofile_log_ex(ddp_mmp_get_events()->present_fence_release,
					       MMPROFILE_FLAG_PULSE, -1, 0x4a4a4a4a);
				/* DISPDBG("_get_sync_info fail in present_fence_release thread\n"); */
				continue;
			}
...
int primary_display_init(char *lcm_name, unsigned int lcm_fps)
{
	...
	DISPPRINT("primary_display_init begin\n");
...
#ifndef MTK_FB_DFO_DISABLE
	if ((dfo_query("LCM_FAKE_WIDTH", &lcm_fake_width) == 0)
	    && (dfo_query("LCM_FAKE_HEIGHT", &lcm_fake_height) == 0)) {
		pr_debug("[DFO] LCM_FAKE_WIDTH=%d, LCM_FAKE_HEIGHT=%d\n", lcm_fake_width,
			 lcm_fake_height);
		if (lcm_fake_width && lcm_fake_height) {
			if (primary_display_change_lcm_resolution(lcm_fake_width, lcm_fake_height) != DISP_STATUS_OK)
...
int primary_display_wait_for_vsync(void *config)
{
	...
	if (pgc->state == DISP_SLEPT) {
		DISPCHECK("VSYNC DISP_SLEPT Return\n");
...
#ifdef CONFIG_SINGLE_PANEL_OUTPUT
int primary_suspend_release_ovl_fence(struct disp_session_input_config *session_input)
{
	...
	for (i = 0; i < session_input->config_layer_num; i++) {
		...
		struct disp_input_config *input_cfg = &session_input->config[i];
...
int primary_display_suspend(void)
{
	...
	if (dpmgr_path_is_busy(pgc->dpmgr_handle)) {
		...
		mmprofile_log_ex(ddp_mmp_get_events()->primary_suspend, MMPROFILE_FLAG_PULSE, 1, 2);
	...
	*
	...
	if (!pgc->force_on_wdma_path) {
		...
		mmprofile_log_ex(ddp_mmp_get_events()->primary_suspend, MMPROFILE_FLAG_PULSE, 0, 3);
	}
	...
	mmprofile_log_ex(ddp_mmp_get_events()->primary_suspend, MMPROFILE_FLAG_PULSE, 0, 4);

#ifdef	CONFIG_SINGLE_PANEL_OUTPUT
	primary_suspend_clr_ovl_config();
#endif
	if (dpmgr_path_is_busy(pgc->dpmgr_handle)) {
		mmprofile_log_ex(ddp_mmp_get_events()->primary_suspend, MMPROFILE_FLAG_PULSE, 1, 4);
...
int primary_display_resume(void)
{
...

	{
		...
		if (_is_decouple_mode(pgc->session_mode)) {
			...
			mmprofile_log_ex(ddp_mmp_get_events()->primary_resume, MMPROFILE_FLAG_PULSE, 2,
				       2);
		}
 ...
 * Defined but not used, avoid build warning. (2015.1.30 Rynn Wu)
 */
static int _config_interface_input(struct primary_disp_input_config *input)
{
	int ret = 0;
	...
	ret = _convert_disp_input_to_rdma(&(data_config->rdma_config), (struct disp_input_config *)input);
 ...
 * Defined but not used, avoid build warning. (2015.1.30 Rynn Wu)
 */
static int _config_ovl_output(struct disp_mem_output_config *output)
{
	int ret = 0;
	struct disp_ddp_path_config *data_config = NULL;
	disp_path_handle *handle = NULL;
...
static int _config_rdma_input(struct disp_session_input_config *session_input, disp_path_handle *handle)
{
	int ret;
	struct disp_ddp_path_config *data_config = NULL;
	...
	ret =
	    _convert_disp_input_to_rdma(&(data_config->rdma_config),
					(struct disp_input_config *) session_input);
	data_config->rdma_dirty = 1;
...
int primary_display_user_cmd(unsigned int cmd, unsigned long arg)
{
	...
	mmprofile_log_ex(ddp_mmp_get_events()->primary_display_cmd, MMPROFILE_FLAG_START,
	...
	mmprofile_log_ex(ddp_mmp_get_events()->primary_display_cmd, MMPROFILE_FLAG_END,
...
int primary_display_switch_mode(int sess_mode, unsigned int session, int force)
{
	...
	mmprofile_log_ex(ddp_mmp_get_events()->primary_switch_mode, MMPROFILE_FLAG_START,
	...
	if (pgc->session_mode == DISP_SESSION_DIRECT_LINK_MODE
		...
		pgc->session_mode = sess_mode;
		DISPMSG("primary display is %s mode now\n", session_mode_spy(pgc->session_mode));
		mmprofile_log_ex(ddp_mmp_get_events()->primary_switch_mode, MMPROFILE_FLAG_PULSE,
			       pgc->session_mode, sess_mode);
		/* primary_display_diagnose(); */
	} else if (pgc->session_mode == DISP_SESSION_DECOUPLE_MODE
	...
	mmprofile_log_ex(ddp_mmp_get_events()->primary_switch_mode, MMPROFILE_FLAG_END,
		       pgc->session_mode, sess_mode);
...
{
	if (handle && (handle == (struct cmdqRecStruct *)pgc->cmdq_handle_ovl1to2_config))
		return 1;
	else
		return 0;
}
...
int primary_display_setbacklight(unsigned int level)
{
	...
	{
		/*
		* extern uint32_t DSI_dcs_read_lcm_reg_v2(enum DISP_MODULE_ENUM module, struct cmdqRecStruct *cmdq,
...
int primary_display_set_cmd(int *lcm_cmd, unsigned int cmd_num)
{
	...
	_primary_path_lock(__func__);
	if (pgc->state == DISP_SLEPT) {
		DISPCHECK("Sleep State set cmd invald\n");
	} else {
		if (primary_display_cmdq_enabled()) {	/* cmdq */
			int ret = 0;
			struct cmdqRecStruct *cmdq_handle_cmd = NULL;
	...
	*va = (unsigned long)ioremap_nocache(pa_start, pa_end - pa_start + 1);
...
#ifndef MTKFB_NO_M4U
	DISPPRINT("disphal_allocate_fb, pa=%pa, va=0x%lx map_size=0x%x\n",
	...
	if (1) {
		...
		*mva = pa_start & 0xffffffffULL;
		...
		DISPPRINT("disphal_allocate_fb, FB MVA is 0x%lx PA is %pa\n", *mva, &pa_start);
...
int primary_display_switch_dst_mode(int mode)
{
	...
	if (lcm_cmd == NULL) {
		DISPCHECK("[primary_display_switch_dst_mode]get lcm cmd fail %d, %d\n",
			  primary_display_cur_dst_mode, mode);
		goto done;
	} else {
		int temp_mode = 0;
