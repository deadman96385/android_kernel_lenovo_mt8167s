...
static int fts_test_save_test_data(char *file_name, char *data_buf, int len)
{
    ...
    sprintf(filepath, "%s%s", FTS_ITO_RESULT_PATH, file_name);
    if (NULL == pfile) {
                        ...
                        bool mc_sc, bool key, bool result)
{
    ...
    strncpy(info->name, name, TEST_ITEM_NAME_MAX);
    info->code = code;
    info->mc_sc = mc_sc;
...
#ifdef GTP_SELF_TEST
#define  TPD_ANDROID_TOUCH   	"android_touch"
#define  TPD_ITO_SELF_TEST     "self_test"
static ssize_t tpd_ito_test_write(struct file *file, const char __user *buffer,
...
{
    tpd_INFO(" %s POWER KEY event %x press\n",  KEY_POWER);
    input_report_key(tpd->dev, KEY_POWER, 1);
    input_sync(tpd->dev);
    tpd_INFO(" %s POWER KEY event %x release\n",  KEY_POWER);
    input_report_key(tpd->dev, KEY_POWER, 0);
    input_sync(tpd->dev);
}
*/
static int tpd_ito_test_show(struct seq_file *file, void* data)
{
	...
	FTS_TEST_DBG("fwname:%s.", fwname);
	printk("[focal] %s()\n", __func__);
	for (i = FT3427_RAWDATA_TEST ; i <= FT3427_PANELDIFFER_TEST;  i++) {
	...
	//Cap Rawdata result
	if (test_data.test_item[FT3427_RAWDATA_TEST].testresult)
		
		{
		    test_result_bmp[MC_RAWDATA] = 'P';
		} 
	else 
		{
			test_result_bmp[MC_RAWDATA] = 'F';
		}
		

	if (test_data.test_item[FT3427_SCAP_CB_TEST].testresult)

	      {
		    test_result_bmp[CB_DATA] = 'P';
	      } 
	else  
		  {
		    test_result_bmp[CB_DATA] = 'F';
	      }
	
	if  (test_data.test_item[FT3427_SCAP_RAWDATA_TEST].testresult)
	...
	if (test_data.test_item[FT3427_PANELDIFFER_TEST].testresult)
	...
	//short test result
	if (test_data.test_item[FT3427_WEAK_SHORT_CIRCUIT_TEST].testresult) 
		{
	...
	seq_printf(file, "0%c-1%c-2%c-3%c-4%c\n",
		test_result_bmp[MC_RAWDATA],
		test_result_bmp[CB_DATA],
...
static int tpd_ito_test_open_on (struct inode* inode, struct file* file)
{
	return single_open(file, tpd_ito_test_show, NULL);
}

static const struct file_operations ito_test_ops_on = {
    .owner = THIS_MODULE,
    .open = tpd_ito_test_open_on,
    .read = seq_read,
    .write = tpd_ito_test_write,
};

static struct proc_dir_entry *tpd_android_touch_proc=0;
static struct proc_dir_entry *tpd_ito_test_proc_on=0;
static int ITO_TEST_COUNT=0;

int tpd_create_ito_test_proc(struct i2c_client *client)
{
	ITO_TEST_COUNT=0;

	tpd_android_touch_proc = proc_mkdir(TPD_ANDROID_TOUCH, NULL);
	tpd_ito_test_proc_on = proc_create(TPD_ITO_SELF_TEST, 0666,
					      tpd_android_touch_proc, &ito_test_ops_on);
		if (!tpd_ito_test_proc_on){
