...
static int fts_irq_registration(struct fts_ts_data *ts_data)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,FT3427");
	if (node == NULL) {
...
static void fts_platform_data_init(struct fts_ts_data *ts_data)
{
	...
	FTS_INFO("max touch number:%d, irq gpio:%d, reset gpio:%dresolution:(%d,%d)~(%d,%d)",
		 pdata->max_touch_number, pdata->irq_gpio, pdata->reset_gpio,
		 pdata->x_min, pdata->y_min, pdata->x_max, pdata->y_max);
}
#ifdef GTP_SELF_TEST
extern int tpd_create_ito_test_proc(struct i2c_client *client);
#endif
			...
			const struct i2c_device_id *id)
{
	...
	tpd_create_ito_test_proc(client);
#endif
