...
static u8 tpd_proximity_flag = 1; //flag whether start alps
				...
				unsigned int addr,unsigned int len)
{
	...
	print_info("gsl_read_oneframe_data:::addr=%x,len=%x\n",addr,len);
	for(i=0;i<len/2;i++){
...
static int gsl_test_i2c(struct i2c_client *client)
{
	int i,err;
	u8 buf[4]={0};
	print_info("[tp-gsl]yjz gsl_test_i2c init. \n");
	for(i=0;i<5;i++)
	{
	print_info("[tp-gsl]yjz gsl_test_i2c 123. \n");
		err=gsl_read_interface(client,0xfc,buf,4);
		print_info("[tp-gsl]yjz gsl_test_i2c 456. \n");
		if(err>0)
		{
...
static void gsl_sw_init(struct i2c_client *client)
{
	...
	tpd_gpio_output(tpd_rst_gpio, 0);	
	msleep(20);
	tpd_gpio_output(tpd_rst_gpio, 1);
	msleep(20);
#endif
...
static void gsl_timer_check_func(struct work_struct *work)
{
	...
	gsl_read_interface(ddata->client,0xac,read_buf,4);
	 print_info("gslX680   buf[0] = %d buf[1] = %d,  buf[2] = %d  buf[3] = %d \n",read_buf[0],read_buf[1],read_buf[2],read_buf[3]);
	
	 if (read_buf[0] == 1 && read_buf[1] == 0 && read_buf[2] == 0 && read_buf[3] == 0)
	 {
	 ...
	 print_info("gslX680  probe ps change	tpd_proximity_detect = %d  \n",tpd_proximity_detect);
	...
	for(i=0;i<5;i++){
		...
		if(read_buf[3] != 0x5a || read_buf[2] != 0x5a || 
			...
			print_info("gsl_monitor_worker 0xb0 = {0x%02x%02x%02x%02x};\n",
...
static int tpd_enable_ps(int enable)
{
	...
	else 
	{
		...
		buf[0] = 0x0;
		gsl_write_interface(ddata->client, 0x00,buf,4);	
		print_info("tpd-ps function is off\n");
	}
	return 0;
}
...
static int ps_enable_nodata(int en)
{
	...
	print_info("gsl_obj ps enable value = %d\n", en);
	if (en) {
		tpd_enable_ps(1);
...
static ssize_t gesture_mode_store(struct class *class, struct class_attribute *attr,const char *buf, size_t count)
{
	...
	print_info(KERN_ERR"dingxueqi test gestrue_en%d\n",gestrue_en);
...
//#if defined(DROI_PRO_PU6A_DNT_P09)
	gsl_read_interface(ddata->client, 0x80, tmp_buf, 4);
	cinfo.finger_num = tmp_buf[0]&0x0f;
	if(cinfo.finger_num > 0)
		gsl_read_interface(ddata->client, 0x84, &tmp_buf[4], 8);
	if(cinfo.finger_num > 2)
		gsl_read_interface(ddata->client, 0x8c, &tmp_buf[12], 8);
	if(cinfo.finger_num > 4)
		gsl_read_interface(ddata->client, 0x94, &tmp_buf[20], 8);
	if(cinfo.finger_num > 6)
		gsl_read_interface(ddata->client, 0x9c, &tmp_buf[28], 8);
	if(cinfo.finger_num > 8)
		gsl_read_interface(ddata->client, 0xa4, &tmp_buf[36], 8);
//#endif
	//cinfo.finger_num = tmp_buf[0] & 0x0f;
	#ifdef GSL_GESTURE
		print_info("GSL:::0x80=%02x%02x%02x%02x\n",tmp_buf[3],tmp_buf[2],tmp_buf[1],tmp_buf[0]);
...
//		for(tmp=0;tmp<3;tmp++){
//			print_info("tp-gsl-gesture 0x%2x=0x%02x%02x%02x%02x;\n",
...
//#define GSL_C		100
//#define GSL_CHIP_1	0xffc8a500  //huaxin 
//#define GSL_CHIP_2	0xffe8a400  //mwd
...
static int of_get_gsl_platform_data(struct device *dev)
{
	print_info("xuke: %s\n", __func__);
	if (dev->of_node) {
		...
		if (!match) {
			print_info("Error: No device match found\n");
			return -ENODEV;
		}
		tpd_rst_gpio = of_get_named_gpio(dev->of_node, "rst-gpio", 0);
		tpd_int_gpio = of_get_named_gpio(dev->of_node, "int-gpio", 0);
		print_info("xuke: %s, tpd_rst_gpio=%d, tpd_int_gpio=%d\n", __func__, tpd_rst_gpio, tpd_int_gpio);
	}

	print_info("xuke: %s done\n", __func__);
	return 0;
}

static int gtp_get_gpio_res(struct device *dev)
{
	int ret;

	print_info("xuke: %s\n", __func__);
	/* configure the gpio pins */
	ret = devm_gpio_request_one(dev, tpd_rst_gpio, GPIOF_OUT_INIT_LOW,
			       "touchp_reset");
	if (ret < 0) {
		print_info("Unable to request gpio reset_pin\n");
		return -1;
	}
	ret = devm_gpio_request_one(dev, tpd_int_gpio, GPIOF_IN, "tpd_int");
	if (ret < 0) {
		print_info("Unable to request gpio int_pin\n");
	}
	print_info("xuke: %s done\n", __func__);
	return 0;
}
...
static int  gsl_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	{
		client->addr = 0x40;
		print_info("Crystal_shen:i2c_client_GSL->addr=%d\n",client->addr);
	}
...
#if 1
	of_get_gsl_platform_data(&client->dev);
	ret = gtp_get_gpio_res(&client->dev);
	if (ret < 0) {
		print_info("Failed to get gpio resources");
		return ret;
	}
#endif
	gsl_hw_init();
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,GSL2338");
	if (node) {
...
static void gsl_suspend(struct device *h)
{
	...
	print_info();
	print_info("gsl_suspend11111111111111111111\n");
	print_info("gsl_suspend:gsl_ps_enable = %d\n",gsl_ps_enable);
#if defined(MX2116_JC_C12)||defined(MX2116_JC_C11)||defined(MX2116_WPF_D5019)  	
#if  0//def GSL_TIMER	
	cancel_delayed_work_sync(&gsl_timer_check_work);
	i2c_lock_flag=0;
...
#ifdef GSL_GESTURE
	print_info("kkkkkkkk gestrue_en=%d",gestrue_en);
	if(gestrue_en){
	...
	#ifdef CONFIG_OF_TOUCH
	print_info("[tpd-gsl][%s][%d] eeeeeeeeeeeeeeeeeeeeeeeeeee\n",__func__,__LINE__);
	disable_irq(touch_irq);
	#else
	mt_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
	#endif
