...
static void gup_search_file(s32 search_type)
{
	...
	for (i = 0; i < GUP_SEARCH_FILE_TIMES; ++i) {
		...
		if (search_type & AUTO_SEARCH_BIN) {
			...
			if (IS_ERR(pfile)) {
				pfile =
				    filp_open(UPDATE_FILE_PATH_2, O_RDONLY, 0);
				if (!IS_ERR(pfile)) {
		...
		if ((search_type & AUTO_SEARCH_CFG)
			...
			if (IS_ERR(pfile)) {
				pfile =
				    filp_open(CONFIG_FILE_PATH_2, O_RDONLY, 0);
				if (!IS_ERR(pfile)) {
					GTP_INFO
				...
				struct st_fw_head *fw_head, u8 *path)
{
	...
	if (path) {
		...
		ret =
		    request_firmware(&update_msg.fw, GT9XX_FW_NAME,
				     &client->dev);
		if (ret < 0) {
...
static u8 gup_load_section_file(u8 *buf, u32 offset, u16 length, u8 set_or_end)
{
	...
	if (got_file_flag == HEADER_FW_READY) {
		if (set_or_end == SEEK_SET)
			...
			memcpy(buf, &gtp_default_FW[update_msg.fw_total_len +
...
static u8 gup_burn_fw_link(struct i2c_client *client)
{
	...
	ret =
	    gup_burn_fw_app_section(client, fw_link, 0x9000, FW_SECTION_LENGTH,
				    0x38);
	...
	ret =
	    gup_load_section_file(fw_link, offset,
				  FW_LINK_LENGTH - FW_SECTION_LENGTH, SEEK_SET);
	...
	ret =
	    gup_burn_fw_app_section(client, fw_link, 0x9000,
...
static u8 gup_burn_fw_app_code(struct i2c_client *client)
{
	...
	u8 retry = 0;
	s32 ret = 0;
	u16 start_index = 4 * FW_SECTION_LENGTH + FW_DSP_LENGTH + FW_BOOT_LENGTH + FW_BOOT_ISP_LENGTH + FW_LINK_LENGTH;
	/* u16 start_index; */
	...
	ret =
	    gup_load_section_file(fw_app_code, start_index, FW_SECTION_LENGTH,
				  SEEK_SET);
	if (ret == FAIL) {
		GTP_ERROR
	...
	ret =
	    gup_burn_fw_app_section(client, fw_app_code, 0x9000,
...
static s32 gup_prepare_fl_fw(char *path, struct st_fw_head *fw_head)
{
	...
	if (!memcmp(path, "update", 6)) {
		...
		for (i = 0; i < GUP_SEARCH_FILE_TIMES; ++i) {
			...
			if (IS_ERR(update_msg.file)) {
				update_msg.file =
				    filp_open(FL_UPDATE_PATH_SD, O_RDONLY, 0);
				if (IS_ERR(update_msg.file)) {
					msleep(2000);
					continue;
				} else {
...
static u8 gup_download_fw_ss51(struct i2c_client *client, u8 dwn_mode)
{
	...
	ret = i2c_write_bytes(client, 0xC000, &gtp_touch_fw[FW_HEAD_LENGTH], FW_DOWNLOAD_LENGTH);
			       ...
			       u8 *target_fw, u32 chk_total_length)
{
	...
	while ((checked_times < MAX_CHECK_TIMES)
		...
		if (!i)
			ret =
			    i2c_read_bytes(client, chk_addr, chk_buf, rd_size);
		else
...
static u8 gup_download_fw_dsp(struct i2c_client *client, u8 dwn_mode)
{
	...
	ret = i2c_write_bytes(client, 0xC000, &gtp_touch_fw[FW_HEAD_LENGTH + 2 * FW_DOWNLOAD_LENGTH], FW_DSP_LENGTH);
	if (ret == -1) {
...
s32 gup_load_by_bank(u8 bank, u8 need_check, u8 *fw, u32 length)
{
	...
	while (retry++ < 5) {
		ret =
		    gup_set_ic_msg(i2c_client_point, _bRW_MISCTL__SRAM_BANK,
				   bank);
		if (ret <= 0) {
		...
		if (need_check) {
			ret =
			    gup_check_and_repair(i2c_client_point, 0xC000, fw,
						 length);
			if (ret == FAIL) {
...
s32 gup_load_calibration(u8 *data, s32 length, u8 need_check)
{
	...
	while (length > 0) {
		...
		ret =
		    gup_load_by_bank(bank, need_check,
...
s32 gup_load_authorization_fw(void)
{
	...
	if (section == NULL) {
		section = gtp_hotknot_authorization_fw;
		GTP_INFO
		    ("[gup_load_authorization_fw] Use default calibration2.");
	} else {
		is_load_from_file = 1;
	}
#endif
	...
	length = section[0] * 256 * 256 * 256 +
	    section[1] * 256 * 256 + section[2] * 256 + section[3];

	if (length > 32 * 1024 || length < 4 * 1024) {
		GTP_ERROR
		    ("[gup_load_authorization_fw]calibration2's length is invalid.");
	...
	if (ret == FAIL) {
		GTP_ERROR
		    ("[gup_load_authorization_fw]calibration2's checksum is error.");
		goto load_calibration2_exit;
	}
...
s32 gup_recovery_touch(void)
{
	...
	ret =
	    gup_load_calibration(&section[CALIBRATION_HEADER_LEN],
