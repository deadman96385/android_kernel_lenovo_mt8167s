...
#define GSL_LOGD(fmt, args...)		\
		pr_debug(GSLX680_NAME	"<-dbg-> [%04d] [@%s]" fmt, \
...
static int test_i2c(struct i2c_client *client)
{
	...
	result += gsl_i2c_write_bytes(client, 0xf0, write_buf, sizeof(write_buf));
...
static int tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (get_boot_mode() == RECOVERY_BOOT) {
		GSL_LOGE(TPD_DEVICE " recovery mode do not need touch panel!\n");
		return 0;
	}
