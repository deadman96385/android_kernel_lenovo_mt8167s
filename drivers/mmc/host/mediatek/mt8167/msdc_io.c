	...
	struct msdc_host *host)
{
	...
	if (clk_prepare(host->src_clk_ctrl)) {
		pr_err("can not prepare msdc%d src clock control\n", pdev->id);
		return 1;
	}
	...
	if (clk_prepare(host->bus_clk_ctrl)) {
...
int msdc_dt_init(struct platform_device *pdev, struct mmc_host *mmc)
{
	...
	if (toprgu_reg_base == NULL) {
		np = of_find_compatible_node(NULL, NULL, "mediatek,mt8167-wdt");
